...
>
> **Sist generert:** 2026-09-24 sen kveld (**Febo-runden: bøffelhale, oksemuskel og kaninører PUBLISERT — tre nye tygg live, de første fôr-/godbitproduktene i butikken.** Trainer Mix og hjortegevir venter på bilde. Merkeopplysninger etter Mattilsynet på alle fem, vekter satt, schema-brand fra vendor, Nyhet-badge og llms.txt oppdatert. Detaljer i STATUS-seksjonen under.)
>
> **Forrige generering:** 2026-09-24 kveld (**Hundetips `oppbevaring-torrfor-hund` — leser-drevet tillegg LUKKET: blindkontroll 7 funn + H1/meta/plast-premiss rettet (`ddb5b9f` + `671d1ea`).** FAQ 7, Turek, Kępińska-Pacelik, Gill og FDA/AAFCO bestått. Funn rettet: GI-påstand ut, bakterier ut + lagermidd med Hibberson & Vogelnest 2014, «pakkeingeniører» → FDA/AAFCO, matskål-som-øse attribuert FDA (vask etter hver bruk), «oftere enn folk tror» ut, steg 2 uten fettbarriere-rest, stål/glass + 3–6 mnd ut, 27 °C attribuert FDA 80 °F. Sondres avgjørelser: plast-absorpsjonspremisset skrevet om til FDA «residual fat and crumbs» på ingress/Feil 1/FAQ 1 (ingen primærkilde fantes); H1 → «5 feil som kan gjøre hundematen skadelig» + Admin-tittel via `pageUpdate` (headline matcher); meta description via `metafieldsSet` (144 tegn), alle tre tagger verifisert live. Dobbeltsjekk runde 2: 19 rettet (bl.a. temperaturstabilitet skilt fra FDA, «løs mage» ut). STEG 19 runde 2: 10 rettet. sha256 MATCH, GSC forespurt. Tittelen speilet etter klarsignal (`8286137`): hub-kort `card_30`, `article_map` (2100) og research-brief; tre llms-URL-er + hubben verifisert live, gammel tittel 0 treff. **Ingenting åpent.** Lærdom godkjent: kildesveip av hele artikkelen i Fase 1 ved leser-drevne tillegg i pre-v2-artikler. Raseguidene og Turguiden urørt.)
>
> **Forrige generering:** 2026-09-24 ettermiddag (**Hundetips `oppbevaring-torrfor-hund` — leser-drevet tillegg, ÅPEN, venter på blindkontroll (`ecf9af8` + `d874933` + `f06d1cf`).** Leser-«nei» 06.08 («samboer heller fra en ny stor sekk til en mindre gammel sekk … kontaminerer ny mat»): artikkelen dekket beholder-til-beholder, ikke sekk-til-sekk. Ny FAQ 7 «Kan jeg helle nytt tørrfôr oppi den gamle sekken?» (100 ord, «Det bør du unngå», FDA og AAFCO hver på sin kilde, Gill 2011 med DOI) + sekk-presisering under Feil 4. Scope-utvidelse godkjent: tre ukildede påstander rettet — «FDAs pet food-rapportering → nyre/lever/kreft» erstattet med Turek et al. 2003 (24 valper, oksidert fett → lavere vekt/vitamin E/immunfunksjon), «fettbarrierelag/mylar/Ziploc» → nøytral FDA-ordlyd, «6 uker» → produsentens anbefaling + AAFCO «ikke i månedsvis» + Kępińska-Pacelik 2025 (råfett 18,4 → 16,9 g/100 g TS etter 6 mnd). Produkt-fit: ingen. Tittel/H2 urørt. Dobbeltsjekk 30 funn/25 rettet, STEG 19 på live 18 avvik rettet, FAQ↔JSON-LD 7/7, sha256 MATCH (første push feilet med `socket hang up` — pull-hash avslørte det, retry OK), GSC forespurt. **Åpent:** blindkontroll; H1 «giftig» og «porøs plast»-premisset notert som senere vurdering. Raseguidene og Turguiden urørt. Detaljer i hundetips-STATUS 2026-09-24 og kildelogg i health-claims-register.)
>
> **Forrige generering:** 2026-09-24 (**Labrador Retriever — leser-drevet retting runde 2 LUKKET: blindkontroll 6 funn rettet (`cbef4e4`).** Kjernepunktene (FCI, AKC-vekt, VetCompass, NKK 2024/2025, HD/AD-lister, EIC, POMC, Dittmann, Wallis, Egenvall) bekreftet uavhengig. Funn: «5-minuttersregelen» var kildeløs → Krontveit et al. 2012 (NMBU-kohort, 133 labradorer: trapper 0–3 mnd er HD-risikofaktoren); Raffans flatcoat-allelfrekvens er internt inkonsistent i kilden → «slik studien selv oppgir»; Agria-rangering presisert til de ti vanligste rasene; tre småting. Dobbeltsjekk av de endrede setningene i fersk kontekst: 6 presiseringer før commit. sha256 MATCH, `/llms.txt` 5400, GSC forespurt. Lærdom #71. **Ingenting åpent på Labrador utenom FAQ-pris (egen sak).** Første runde samme dag (`4cc11ec` + `a93c0ff`):** Leserkommentaren fra 21.08 var behandlet 25.08; full audit fant rest-avvik: FCI-temperamentsitatet var AKC-standardens, vekten AKC-lb uten merking, levetid uten nordiske data, EIC-bærerandel uten populasjon, HD/AD framstilt som klubbkrav (er NKK-registreringskrav fra 1989/2018), «working vs show» som tidlig-1900-tall (RAS: i Norge først fra 1990-tallet), «Stifinneren» uten dagens status. Nytt: Agria 1995–2000 (Egenvall 2005) som nordisk feste, NKK-arkene 2024/2025 (823/893, 3. plass), NKK-DNA-oversikten (oppdatert 17.09.2026: prcd-PRA/EIC/HNPK uten krav), RAS Tabell 7, HNPK/SD2, Wallis 2025 DENND1B, kildeseksjon 20 kilder / 8 DOI-er, alle anglisismer oversatt (to H2-er omdøpt, ankre uendret, `article_map` synket). Dobbeltsjekk 25 + STEG 19 22 funn rettet — inkl. et `;` fra meg i `mh-rasedata`-fotnoten som kuttet faktastripa på live i ~40 min. sha256 MATCH, `/llms.txt` 5300, GSC forespurt. Lærdom #67–#70. **Åpent:** blindkontroll; FAQ-pris utenfor scope. Golden, Sort, Grå, Turguiden urørt.)
>
> **Forrige generering:** 2026-09-24 morgen (**Norsk Elghund Sort — valpetall oppdatert til NKK 2023–2025, LUKKET (`8e274eb`).** Kontrollørens sidefunn fra Grå-saken: «rundt 370 valper i året» sto som nåtid fire steder (§Sort vs Grå, Oppsummering, FAQ 8 + JSON-LD) men var RAS-tallene for 2021–2022. NKKs regneark «Mest reg rase 2025.xlsx» åpnet: NES 371 (2022), 271 (2023), 282 (2024), 262 (2025). Rettet til «rundt 260–280 valper i året (NKK 2023–2025)», recap «262 i 2025, mot 371 i 2022», ny nøytral NKK-setning i §Kritisk truet, kildeoppføringen utvidet (inkl. RAS/NKK-avviket for 2021–22: 371/369 mot 362/371). Kun tallbytte — ingen blindkontroll. FAQ↔JSON-LD 8/8, port 8 ikke berørt, sha256 MATCH, fersk render verifisert, GSC forespurt. Lærdom #66 (nåtidstall uten årstall eldes usynlig — skriv alltid «X i ÅÅÅÅ (kilde)»). Grå-guiden og Turguiden urørt. **Ingenting åpent på Sort.**)
>
> **Forrige generering:** 2026-09-23 natt (**Norsk Elghund Grå — leser-drevet retting LUKKET: blindkontroll bestått på kjernepunktene, 5 funn rettet (`056f768`): overvær ≠ sporarbeid, ukildet ettersøks-påstand kildet, 1901/1905 etter SNL, Skrub-datering ut, «lurhund» → «ledhund». Kontrolløren flagget Sort-guidens «370 valper» mot NKK 262 — neste leser-drevne sak, ikke rørt. Første runde `b50e399`:** Oppfølging av Sort-saken: 23/20 kg var ukildet — FCI 242 oppgir kun høyde; NEKF sier hann rundt 21 / tispe rundt 18 kg, NJFF 18–25. I samme sveip: NM-brøkene (kildeløse) erstattet med NKKs regneark «Mest registrerte raser 2025» (NEG 563, 8. plass; jämthund 351); «750–1 200 valper» → NEKF ~750 / NKK 706-631-563 / RAS-snitt 948; levealder fikk primærkilde (Lingaas 2021 NEG-rapporten funnet via NES-rapportens URL-mønster: 122 døde hunder, snitt 10,6 år) med forbehold; HD «76,7 %» → Lingaas 81,5 % + NKK-krav A/B/C fra 01.04.2026; indeksavl 2016 (ikke 2018); glaukom 6,5 år (Ahonen 2014, DOI); gråhundsjuken «anekdotisk» etter NKK; «rektangulær profil» → «kvadratisk» (FCI); ny kildeseksjon med 12 kilder. Økta ble clearet etter godkjent Fase 1 — STEG 0 ved gjenopptak viste rent repo, ingenting pushet. Dobbeltsjekk i fersk kontekst: 14 funn rettet; STEG 19 på preview-render (cookie-jar, kontrolltest): 21 avvik rettet FØR live. sha256 MATCH, fersk render verifisert, `/llms.txt` 4850, GSC forespurt. Lærdom #61–#65 i raseguider/sprint-lessons.md. **Ingenting åpent på Grå.**)
>
> **Forrige generering:** 2026-09-23 sen kveld (**Norsk Elghund Sort — leser-drevet retting LUKKET: blindkontroll 3 funn rettet (`ff47bae`).** Kontrolløren bekreftet vekt (NEKF 15/13 + NJFF 13–20), høyder, King-omregning, populasjon/Ne, HD/gentest-krav, Berg-studiene og FAQ uavhengig. Tre funn: 1895-standarden flagget som «sprik» mot SKKs RAS (forslag 1901) — dobbeltsjekk viste at NEKFs RAS selv har begge årstallene (to hendelser, ikke sprik; Sondre godkjente løsningen); 1877 presisert til «samme klasse som de andre spisshundene»; King-pronomen rettet. sha256 MATCH, fersk render verifisert, GSC forespurt. **Ingenting åpent på Sort. Neste leser-drevne retting: Grå-guidens 23/20 kg mot NEKFs 21/18.** Lærdom #56–#60 i raseguider/sprint-lessons.md.)
>
> **Forrige generering:** 2026-09-23 sen kveld (**Norsk Elghund Sort — leser-drevet retting, ÅPEN, venter på blindkontroll.** Leserkommentaren fra 03.09 («20–23 kilo er enten altfor stor eller veldig feit») var riktig: tallet var Grå-guidens (23/20 kg), ingen primærkilde bar det — FCI 268, RAS v4, NKK, NIBIO og SKK oppgir kun høyde. Rettet til NEKFs 15/13 kg (hann/tispe) + NJFF 13–20 på fire flater; King regnet om («rundt tre ganger så tung»). I samme sveip: «en av Norges sjeldneste» → «en av sju nasjonalraser, kritisk truet (Norsk genressurssenter)», Grå-pels «fluffy» → FCI 242 «stri overpels», ukildede AUNES-/FAO-attribusjoner ut, HD-indeks 203 → 200 (FS-vedtak nov. 2024), ny kildeseksjon (21 kilder, 3 DOI-er i schema), ny faktastripe-rad, lesetid 16 → 23. Dobbeltsjekk 11 punkter + STEG 19 15 avvik, alle rettet (`4631363` + `3cece0c`), sha256 MATCH, GSC forespurt. **Neste leser-drevne retting: Grå-guidens 23/20 kg mot NEKFs 21/18.** Detaljer i raseguider/STATUS.md.)
>
> **Forrige generering:** 2026-09-23 kveld (**Golden Retriever — leser-drevet retting LUKKET: blindkontroll bestått, 1 skrivefeil rettet (`8c34651`), meta description satt via metafieldsSet og verifisert.** Tidligere samme dag: `95a86a3` + STEG 19 fix-forward `33b0301`. Leserkommentaren fra 21.08 var allerede rettet i august; full audit fant likevel rest-avvik: irsk setter-tallet feilattribuert til Kent 2018, null nordiske kreftdata, ukildet vekt og levetid, McMillan-sammenligning på kryss av studier. Nytt: Bonnett 2005 + Engdahl 2025 (Agria) som nordisk feste, KC 2004-median, «typisk vekt» med FCI/AKC-forbehold, kildeseksjon med 19 kilder og 9 DOI-er i schema. STEG 19 fant at steg-blokken i §Pelsstell aldri hadde hatt CSS (løse sifre siden mai) — fikset. **Ingenting åpent.** Detaljer i raseguider/STATUS.md 2026-09-23.)
>
> **Forrige generering:** 2026-09-22 (**Oppgave 11 LUKKET — begge Admin-stegene er på plass og hele kjeden er verifisert ende-til-ende.** Friday la abonnementet på `turguiden_cta_click` inn i Custom Pixel 150765646 (tilfelle B — fetch-mønsteret fra `product_callout_click` kopiert med de seks parameterne, ingen eksisterende kode endret) og opprettet URL-redirecten `/turguiden`. Målt 22.09: ett klikk på `/pages/vizsla` gir **to** GA4-kall — Measurement Protocol fra pikselen og `/g/collect` fra gtag — og GA4 Realtime på property 519627545 viser `turguiden_cta_click` = 2 fra det ene klikket (gotcha #95, del rå-tallet på to). `/turguiden` svarer 301 med alle fire UTM-er. **Bifunn rettet samme dag:** `variant` og `utm_content` var ikke registrert som custom dimensions i GA4 — og registrering er ikke retroaktiv, så splittene ville vært tapt. Begge opprettet via Admin API (13 av 50 event-scope-dimensjoner i bruk). `link_url` bevisst ikke registrert — for høy kardinalitet. **Ingenting gjenstår i Admin.** Detaljer i STATUS-oppføringen 2026-09-21 natt (oppgave 11).)
> 
> **Forrige generering:** 2026-09-21 sen kveld (**Oppgave 8 — intern sjargong og SEO-titler ryddet i én runde: «moat»/«TRIPPEL» ute av synlig tekst på 31 raseguider, 48 anker-ID-er + 48 TOC-lenker omdøpt til norsk på 22 guider, 63 nye SEO-titler (alle ≤60 tegn og unike), `Article.description` henter nå meta-beskrivelsen i stedet for sidetittelen, og ny `scripts/jargon-guard.py` i pre-commit.** Detaljer i STATUS-oppføringen 2026-09-21 sen kveld.)
>
> **Forrige generering:** 2026-09-21 kveld (**Turguiden på forsiden LIVE (oppgave 6d): kompakt stripe mellom «Hundetips fra King» og «Møt King» — aldri over eller mellom hero og første produktrad. Ny `sections/turguiden-home-strip.liquid` med `"settings": []`; `templates/index.json` fikk bare oppføringen + plassen i `order`, ingen markup, så temaeditoren kan flytte/fjerne men ikke endre. `turguiden-varsel-cta` fikk to nye parametre med default (`medium`, `layout`) — de 16 boksene og 77 sidebar-CTA-ene er bit for bit uendret, verifisert før/etter push på seks sider. `utm_medium=home`, `utm_content=forside`, `/turer#fylker`, `data-mh-position="home"`, knappetekst som boksene. Guard-sjekk 7 + unntak i sjekk 1; fem negativtester slår ut.** Detaljer i STATUS 2026-09-21 kveld (6d).)
>
> **Forrige generering:** 2026-09-21 ettermiddag (**Turguiden på produktsider LIVE (oppgave 6c): én brødtekstlinje med grønn understreket lenke på TurPakken, Potevasker og Ullgenser, rett etter accordions og før relaterte produkter — aldri over kjøpsknappen, aldri i sticky-baren. Ny `snippets/turguiden-product-line.liquid` (case på handle, ukjent handle = tomt), `utm_medium=product`, `utm_content=<handle>`, `/turer#fylker`, `data-mh-position="product"`. Guard-sjekk 6 vokter «maks én lenke per produktmal, kroken kun i de tre». Valpepakken vurdert og strøket av Sondre; 18 andre maler NEI. Målt live 390/1280: tekst + href eksakt, ingen horisontal scroll.** Detaljer i STATUS 2026-09-21 (6c).)
>
> **Forrige generering:** 2026-09-21 (**Sticky-fiksen fra gotcha #111 LIVE: `overflow-x: clip` på html/body i theme.liquid, verifisert uten horisontal scroll på 10 sjekker. Turguiden sidebar-CTA LIVE på 77 A/B-artikkelsider (oppgave 6a): kompakt boks (174 px) øverst i sidebar-sticky over TOC, «Få varsel når båndtvangen starter der du bor» → `/turer#fylker`, `utm_medium=article`, `utm_content=<handle>-sidebar`. Kroken står i alle 145 sidebar-filer; snippeten `turguiden-sidebar-cta` avgjør selv via A/B-liste + boks-liste, vokta av `scripts/turguiden-sidebar-guard.py` (0 avvik). Aldri på C (53), aldri på de 16 boks-sidene. GA4-lytter IKKE utvidet — parkert til avlesing medio oktober. Full A/B/C-tabell nå i repoet (vedlegg B i kartleggingen).** Detaljer i STATUS 2026-09-20 natt (6a).)
>
> **Nest forrige:** 2026-09-20 sen kveld (**Turguiden nød-ID-kort for hund BYGD på gren `nod-id-kort` (`626638d`, pushet) — IKKE live: venter på Googles godkjenning av Android-appen + at Sondre oppdaterer App Privacy/Data safety med telefonnummer, og at han limer inn migrasjonen `20260920190000_hund_nodkort.sql` selv. QR på selen → `/h/<kode>` med navn, bilde, rase, telefon, chip; av = 404; telefon ikke trykt på kortet.** Detaljer i STATUS 2026-09-20 sen kveld.)
>
> **Forrige generering:** 2026-09-20 (**Turguiden UTM-harmonisering LIVE (runde 5): alle 13 eldre tekstlenker i 8 seksjoner har nå `utm_content=<handle>-inline`; boksene beholder bar handle. Kun utm_content endret. Rest: `bandtvang-norge` har to tekstlenker til `/turer` som deler URL, og topp-/bunnboks deler URL — venter på vedtak om tredje mønster. Topp 15 A-sider ferdig tidligere samme dag (`bf63677`, `85f638f`, `1e262a6`).** Detaljer i STATUS-oppføringen 2026-09-20 natt (runde 5).)
>
> **Forrige generering:** 2026-09-20 (**Turguiden-lenking runde 3 LIVE (`85f638f`): varsel-CTA + inline tekstlenke på rad 4–8 av topp 15 A-sider — `hund-bader-ute`, `hund-om-sommeren`, `gordon-setter`, `beagle`, `labrador-retriever` (samlet 34 635 visn./90 d). Runde 2 (rad 1–3) samme dag, `bf63677`. Alle CTA-er → `/turer#fylker` (ingen kommune navngitt); to plasseringsavvik fra forslaget (bader-ute, sommer) fordi foreslått seksjon manglet / bar produktkortet. Rad 9–15 ikke rørt.** Detaljer i STATUS-oppføringen 2026-09-20 kveld.)
>
> **Forrige generering:** 2026-09-09 natt (**CTR-analyse av title/meta-sveipet: +14 % mot kontrollens −24 %, posisjon flat. hund-vil-ikke-spise reversert, fire familieraser fikk hverdagstall-titler. Neste avlesing 22.09.** Detaljer i STATUS 2026-09-09 natt.)
>
> **Forrige generering:** 2026-09-09 kveld (**Badebørste til hund – PawWash (`badeborste-hund`) PUBLISERT, sjampobørsten ARKIVERT med 301 i ett hopp; sju artikler pekt om + nytt produktkort i pelsskifte-hund. Gjenstår for Sondre: lager 9, bilder, video-URL, King-tips, RRT, pelsfjerner-pakken.** Detaljer i STATUS-oppføringen 2026-09-09.)
>
> **Forrige generering:** 2026-09-08 kveld (**Poteklipper/PawTrim PUBLISERT og verifisert live — nytt produkt nr. 17, 399 kr.** Detaljer i STATUS-oppføringen 2026-09-08 kveld.)
>
> **Forrige generering:** 2026-09-04 natt (**Produktkort-arbeidet er ferdig i begge korpus.** beste-pelsfjerner-2026 bygget etter malen (men siden 301-redirecter — pelsfjerner-cluster P4). Samlet sluttrapport i `docs/produktkort-utrulling.md`: 96 sider med kort (pelsfjerner 42, SnackSpin 24, akt.skål 6, TurPakken 5, hundeseng 5, CalmBall 4, Valpepakken 3, ullgenser 3, sjampobørste 2, potevasker 2, Andefanten 1), 30 ingen fit (21 adjacency, 3 uforankret, 5 Sondres avgjørelse, 1 selvmotsigende), 413 668 av 536 897 behandlede visninger (77 %) dekket, 20 regler.)
>
> **Forrige generering:** 2026-09-04 natt (**Hundetips fase 2+3 pulje 5 live: de siste ni behandlet — sju kort migrert, valpens-utvikling og hund-lugger-i-band = ingen fit. Alle 45 hundetips-artikler med brødtekst-kort er nå ferdig behandlet: 35 med kort, 10 ingen fit.** Commit: se `git log`.)
>
> **Forrige generering:** 2026-09-04 natt (**Hundetips fase 2+3 pulje 4 live: åtte kort migrert, hund-hopper = ingen fit (funnet av kontrollpasset). 36 av 45 behandlet — pulje 5 (9) gjenstår.** Commit `ccc73ea`.)
>
> **Forrige generering:** 2026-09-04 natt (**Hundetips fase 2+3 pulje 3 live: tre kort migrert, seks flater fjernet — tre adjacency-stopp funnet i fila (hvor-mye-vann, hund-i-bil, eldre-hund). 27 av 45 behandlet.** Commit `7625f7a`.)
>
...
>
> **Forrige generering:** 2026-09-04 natt (**Hundetips touchpoint-opprydding fase 2 pulje A: seks artikler på regelen «ett produkt i kort og bunn-CTA, maks tre tekstlenker, dubletter og forbudsrester ut» — hund-smerte-tegn mistet alle flater (kort, CTA og lenke motsa hverandre), ferie-duoen ned til Potevasker, blor-fra-poten-bunnen 3 → 1.** Kartlegging av alle 71: 29 med avvik, 42 uten. Commit `a61ee48`.)
>
> **Forrige generering:** 2026-09-04 natt (**Hundetips produktkort-opprydding fase 1: seks brødtekst-kort fjernet fra helsetunge artikler etter adjacency-kriteriet, hund-kaster-opp beholdt; hund-darlig-ande mistet også uforankret SnackSpin-bunn-CTA.** Kartlegging av alle 71 hundetips: 57 har produktflate, 14 ingen; ingen bruker `mh-product-callout` ennå. Commit `04a60bc`.)
>
> **Forrige generering:** 2026-09-04 sen kveld (**Golden migrert — produktkort-utrullingen er komplett: 74 av 74 raseguider på malen (61 brødtekst-kort, 71 ett bunn-kort, 3 uten bunn-seksjon, 0 med tre). A/B-kortet på golden avlest i GA4 før fjerning: tapt måling (13 dagers levetid, én sosial trafikk-topp, og GA4 mangler egendefinerte dimensjoner for placement_type/position/breed_page — gotcha #92). Dimensjonene REGISTRERT samme kveld etter at Sondre ga service-kontoen Editor — oktober-målingen kan splitte per plassering fra 04.09 sen kveld; alt før er tapt oppdeling.** Se STATUS og `docs/produktkort-utrulling.md`.)
>
> **Forrige generering:** 2026-09-04 kveld (**Ett produkt i bunn gjelder alle 74 raseguider — de 12 «ingen fit»-guidene trimmet: 10 live med ett bunn-kort (pelsfjerner 7, SnackSpin 2, TurPakken 1); whippet og greyhound har ingen forankret produkt og går til Sondre (fjerne bunn-seksjonen?). Westie trimmet til ett kort (hundesengen). Sondre avgjorde deretter: whippet, greyhound og coton får bunn-seksjonen FJERNET — null er utfallet når ingenting er forankret. 70 av 74 har ett bunn-kort, 3 har ingen. Neste: golden i egen økt.** Se STATUS og `docs/produktkort-utrulling.md`.)
>
...
>
> **Forrige generering:** 2026-09-04 natt (**Produktkort-mal `mh-product-callout` for raseguider godkjent og låst — pilot samojedhund live med kort på 35,3 %, bunn-seksjon redusert til ett produkt, navngitt knappetekst som ny kanon.** Se STATUS.)
>
> **Forrige generering:** 2026-09-04 natt (**dateModified-løpet: ISO-dato på linje 1 driver synlig dato + schema på 146 artikler, push-vakt, AI-synlighet-audit med 5 hull — se STATUS.** Tidligere samme dag: **Julegave-artikkelen `julegave-til-hund` live og lukket — hundetips #71, korpusets første kommersielle gaveguide; kolleksjonen i hovedmenyen; blindkontroll 15 → 14 rettet; gotcha #91 (zsh ord-splitting).** Se STATUS.)
>
> **Forrige generering:** 2026-09-03 kveld (**Julegave-kolleksjonen `julegaver-til-hund` opprettet + publisert via Admin API; temadel på preview; `shopauth` utvidet med publications-scopes — gotcha #87.** Se STATUS.)
>
> **Forrige generering:** 2026-09-01 sent (**Fire globale ytelsesfikser live (`fonter`-commit): temafonter Inter/Barlow/Anonymous Pro av → DM Sans/Playfair same-origin; Google Fonts trimmet til DM Sans + Playfair 500/600 og lastet asynkront; footer-@import fjernet; Judge.me-embed vurdert og beholdt (forsiden bruker karusell + stjerne-snippets). Pluss head-preload av hero-bildet.** Lighthouse mobil live: devtools-LCP 7,6 s → 2,3–3,5 s, score 71–73 → 88–95; PSI-simulert LCP uendret ~11 s (skript- og stilark-graf, gotcha #83/#84). Tidligere samme kveld: konverteringsaudit levert + forsidevideo 43,9 MB → statisk bilde (`56504f5`).)
>
...
**Tillegg 2026-09-08 sent — «Meld fra når på lager» LIVE på alle 16 PDP-er.** Delt snippet `snippets/mh-restock-form.liquid` (Shopifys `{% form 'customer' %}`, tag `restock-<handle>` automatisk, valgfri `sub`/`success`-tekst; rendrer kun ved utsolgt) + to linjer per mal (render-kall etter kjøpsskjemaet, sticky-knapp på mobil → «Meld fra»-lenke ved utsolgt). Rullet ut i tre puljer (poteklipper → 5 enkle maler → 10 maler med varianter/egen sticky-tilstand), hver verifisert på preview-tema 149856485454 med et draft-testprodukt (slettet etterpå) før live-push. Innsending bekreftet manuelt av Sondre (kunde med tag + samtykke). Standardteksten lover «så snart den er tilbake», aldri «samme dag»; ullgenser og slikkematte-ball sier «hvis den kommer tilbake». Potevaskers «Utsolgt — kommer snart» → «Utsolgt». Bifunn: alle 28 varianter har lagersporing PÅ; `aktiviseringsleke-for-hund` er ARCHIVED (products.md sa ACTIVE). **Åpent:** variantnivå (ullgenser à 1 stk per størrelse, hundeseng Large 1). Fasit: `docs/product-templates.md` → «Meld fra når på lager», gotcha #99.


## STATUS — 2026-09-24 sen kveld (Febo-runden — tre tygg publisert) (**Bøffelhale (`boffelhale-hund`, 49 / 3 for 129), oksemuskel (`oksemuskel-hund`, 59 / 3 for 149) og kaninører med pels (`kaninorer-hund`, 79) er ACTIVE og publisert til Online Store — live 200 ×3, indeksering bedt om. Trainer Mix (149) og hjortegevir Easy (M 249 / L 329) står som DRAFT til Sondre har lastet opp minst ett bilde; alt annet på dem er klart.** Fase 1-sjekken (Admin API + rendret preview ×5) ga: priser/varianter korrekte, King-tips tom og skjult, meta ×3 identiske, Product/ProductGroup + FAQPage 6/6 rene, ingen claim-brudd — men vekt 0 på alle sju varianter, ingen fôrtype/ansvarlig firma på noen, oksemuskel uten produsent, Trainer Mix uten sammensetning. **Merkeopplysningene** (Sondre/web-Claude leste pakkene) ligger nå som rader i `tg_spec_rows` på alle fem: Fôrtype (fôrmiddel / tilskuddsfôr), Sammensetning, Tilsetningsstoffer, Analyse, Nettovekt, Oppbevaring, Ansvarlig firma med adresse (Trixie Tarp, Hofman Animal Care Enter, Farm Food Nijverdal — sistnevnte hentet fra farmfood.nl) og Importør Febo Norge AS, Slitu (Brreg-verifisert) der varen er bulk. Regelen er låst i `docs/products.md` → Content Rules. Trainer Mix inneholder fargestoffer og propylenglykol → hard regel mot «naturlig/sunn». Oksemuskel er Trixie (vendor + produsent-metafelt satt). **Schema-`brand`** kommer nå fra `product.vendor` i `mh-product-schema` (fallback Min Hund — nullendring for de 18 gamle). **Vekter satt permanent** (fast fraktpris): 80/250, 20/70, 130, 580, 110/150 g. Lenkene til geviret fra de tre publiserte er DRAFT-sikret og rendres som tekst til det publiseres. `mh_nyhet_handles` ×2 og `llms-products-data` (17 produkter) pushet og verifisert 3/3 på begge llms-rutene. STEG 19 på live: 0 innholdsavvik, 1 konsistenspunkt rettet fix-forward. King-tips holdes tom; King har smakt oksemuskel. **Gjenstår:** lager via Friday (fast fordeling 1 stk / 3 stk: bøffelhale 18/4, oksemuskel 25/8; kaninører 12; Trainer Mix 5; gevir 2/2 — rebalanseres manuelt), bilder + publisering av Trainer Mix og gevir.)

## STATUS — 2026-09-23 (Golden Retriever — leser-drevet retting) (**LUKKET: blindkontroll bestått, 1 skrivefeil rettet i `8c34651`; meta satt og verifisert.** Oppdraget kom med leserkommentaren fra 21.08 som premiss; `git log` viste at begge punktene var rettet 22.–25.08, så auditen dekket hele guiden i stedet. Phase 1-rapport → STOP → godkjent (vekt beholdes som «typisk», kildeseksjon ja, råd-tall som råd) → Phase 2. To uavhengige lesninger i fersk kontekst (dobbeltsjekk før commit: 24 punkter, 9 rettet; STEG 19 på live-render: 21 avvik, fix-forward). Ordtall 3537 → 3871 redaksjonelt. GSC forespurt. Lærdom #52–#55 i sprint-lessons, gotcha #126. Blindkontrollen bekreftet Engdahl/Bonnett/Arendt/GRCA/Kent uavhengig. Meta description satt med `metafieldsSet` etter godkjenning og verifisert tegn for tegn i alle tre tagger + Article-schema.)

## STATUS — 2026-09-21 natt (Turguiden uten produkter) (**Alle produkter fjernet fra Turguiden — LIVE på main, merge `c891f5f`, prod-deploy READY, gren `fjern-produkter` slettet.** **Beslutning (Sondre):** Turguiden skal bygge tillit og ikke oppleves som en butikk; produktlenkene stoppet innlegg i flere Facebook-grupper. **Kartlagt først:** produkter fantes bare ett sted — «Ta med på tur» i turdetaljpanelet (`ta-med-paa-tur.tsx`: vannflaske 3-i-1 + potevasker, 56 px miniatyrer, merkingen «Lenker til Min Hund — Turguidens egen nettbutikk»), vist på alle 4953 turer. Varsel-e-poster, delekort/OG-bilder og øvrige sider hadde ingen produkter (bare avsenderadresser og domenenavn). **Fjernet (commit 4a98e18):** seksjonen, `lib/turutstyr.ts` (bratt/lang-regelen), `public/produkter/*.jpg`, `tester/enhet/turutstyr*.test.ts`. `lib/butikk.ts` og `butikk-lenke.tsx` trimmet til bunnlenka alene (`ButikkKilde` = «fotlenke», `produkt`-prop borte, hendelsen `butikklenke` sender bare `kilde`); `BUTIKK_OPPHAV` lever videre for `raseguideLenke()` i `lib/hund.ts`. **Bunnlenka (commit 659915f, etter Sondres godkjenning):** «Min Hund-butikken» → `/` byttet til «Hundetips fra Min Hund» → `/pages/hundetips`, samme fane, samme UTM-er. **Beholdt med vilje:** raseguide-lenka fra hundeprofilen og alle artikkel-lenker — innhold, ikke salg. **Docs:** MASTER.md §4 har nytt avsnitt «Ingen produkter i Turguiden» (regel: en `/products/`-sti i Turguiden er en regresjon), §6 punkt om produktminiatyrer fjernet; docs/app.md, capacitor-kartlegging.md, nyhetsbrev.md oppdatert. **Verifisert:** tsc rent, 738/738 enhetstester, lint-feil uendret mot main (72 forhåndseksisterende); preview (bypass-hemmelighet generert og tilbakekalt) og deretter prod: tursider 0 produkttreff, `/produkter/potevasker.jpg` 404, bunnlenke `minhundpet.no/pages/hundetips?utm_source=turguiden&utm_medium=referral&utm_campaign=fotlenke` på /personvern og /kontakt. **Felle underveis:** `git stash pop` mistet index-tilstanden, så første commit (9ec3e1d) manglet de seks slettingene og Vercel-bygget feilet; rettet med amend → 4a98e18. **Konsekvens for Min Hund:** Vercel-hendelsen `butikklenke` med `produkt`/`ta-med-paa-tur` slutter å komme; GA4-kilden turguiden/referral får nå bare fotlenke-trafikk til hundetips-huben. Låselista (innlogging, demokonto, kontosletting, Meld fra/Skjul, native, assetlinks, AASA, OAuth, Supabase) urørt.)

> **LUKKET 2026-09-22:** pikselen og redirecten er live (se «Sist generert» øverst). Ingenting gjenstår i Admin.

## STATUS — 2026-09-21 natt (oppgave 11) (**Oppgave 11 — Turguiden-synlighet, åtte punkter i én runde.** Punkt 0–7 levert og verifisert live; punkt 8 blokkert på manglende API-scope.

**0 · UTM-mønster.** `utm_content` er nå `<handle>-<plassering>`: `box-top` · `box-inline` · `box-bottom` · `inline-<n>` · `sidebar` · `home` · `verktoy`. Ankeret sendes ikke til serveren, så to lenker med samme sti var identiske for Vercel — `bandtvang-norge` hadde 8 lenker fordelt på 2 verdier og har nå 8 unike. 22 tekstlenker i 17 filer renummerert. `turguiden-link` fikk valgfri `query` (ferdig-kodede ekstraparametre etter utm-blokka, før ankeret) og `topbar`/`redirect` i medium-allowlista. Guard sjekk 8 krever unik, kanonisk `utm_content` per seksjonsfil; begge negativtester slår ut.

**1 · De siste 23 A-sidene.** Alle 39 A-sider fra vedlegg A har nå CTA-boks. De 23 som bare hadde sidebar (~43 000 visninger/90 d) fikk boks + inline tekstlenke etter samme mønster som de 16 første. `tg_box_handles` 16 → 39, sidebar-CTA 77 → 54. Vann-sidene lenker til `/turer-ved-vann`, resten til `/turer#fylker`. Kommune gjettes aldri. Ny plasseringsregel: boksen settes aldri rett etter et `mh-product-callout` som avslutter en seksjon — da flyttes den til neste H2, ellers stables to bokser (traff `dachshund`, `nova-scotia…`, `hund-om-vinteren`, `engelsk-springer-spaniel`).

**2 · GA4-lytter.** Ny `snippets/mh-track-turguiden.liquid`, rendret **globalt** fra `layout/theme.liquid` — en per-seksjon-krok ville bommet på footeren og forsidestripa. Kanaler: `Shopify.analytics.publish` (bærer), `dataLayer`, `gtag`. Parametre: `placement_type`, `position`, `article_page`, `variant`, `product_handle`, `utm_content`, `link_url`. Verifisert i Playwright på seks flater med riktig payload og 0 konsollfeil fra temaet. **Gjenstår i Admin:** Custom Pixel `150765646` må abonnere på event-navnet — det finnes ingen Admin API for tilpassede piksler. Kodebiten ligger i UTM-standarden runde 12. Les tallene med gotcha #95 (dobbelt-telling).

**3 · Fire temavarianter i sidebar-CTA-en.** De 54 sidene hadde ordrett samme krok og samme mål. Nå: `bandtvang` 35 → `/turer#fylker`, `jakt` 11 → `/logg-inn?…&next=%2Fkonto`, `vann` 3 → `/turer-ved-vann`, `helse` 5 → `/logg-inn?…&next=%2Fkonto`. Jakt- og helse-varianten peker på innloggingssiden fordi varselbryterne for jakt/flått per kommune og påminnelsene for ormekur/flåttmiddel/vaksine per hund ligger på kontosiden — og `/konto` svarer 307 som pakker UTM-ene inn i `next=`-verdien (gotcha #123). Båndtvang beholder `/turer#fylker`: kommunesidenes e-postskjema krever ikke konto. `data-mh-variant` på aside og knapp. Guard sjekk 9.

**4 · Matkalkulatoren.** `/pages/hvor-mye-mat` er butikkens beste CTR-side (19 414 visninger / 323 klikk / 1,66 % siste 90 d). Betinget linje i resultatsteget som vises **kun** når brukeren selv har krysset «Veldig aktiv» — kroken kommer fra brukerens eget svar. `href` bygges server-side, JS skrur av/på `hidden`. Verifisert live: synlig ved «Veldig aktiv», skjult ved «Normal» og «Lite aktiv».

**5 · Vann.** Tekstlinje under TurPakken-CTA-en i vannkalkulatorens resultatboks (aldri en konkurrerende knapp) og tekstlenke i `hund-drikker-ikke-pa-tur` §Hvor mye vann — begge til `/turer-ved-vann`. Begge sto i kartleggingen 17.09 og var aldri gjort.

**6 · Toppstripe.** `snippets/turguiden-topbar.liquid` i to render-kall (head: CSS + avvisningssjekk før første maling; body: markup). Vindu `20261001`–`20261130` som heltall, ikke rendret på `/cart`, lukkbar med localStorage-nøkkel som bærer sluttdatoen. Nøytraliserer «NY»-badgen i nøyaktig samme vindu. **Målt live med stripa midlertidig slått på, deretter slått av igjen:** 390 px 32 px høy og én linje, CLS **0,0000**; 1280 px 36 px, CLS 0,0007; `/cart` ikke rendret; `scrollWidth` = viewport begge steder; 0 konsollfeil. Repo-tilstand `TG_START = 20261001`, live verifisert dormant (0 forekomster i forside-HTML, badgen rendrer `"Ny"` som før).

**7 · llms.txt + 404.** Nytt punkt «Varsler og påminnelser» i alle tre llms-kildene (Trigger D), uten UTM, verifisert på `/llms.txt`, `/llms-full.txt` og `/pages/llms-txt`. 404-siden er oversatt (`Siden finnes ikke`, `Se produktene`) og har fått ny `sections/turguiden-404-hjelp.liquid` med søkefelt mot `product,page,article`, Turguiden-knapp og lenker til Hundetips/Raseguider/Pelstype-veiviser. Egen seksjon fordi Horizons `_search-input` er privat **og** bare søker i `type=product`. **B14 lukket.** Målt: h1 norsk, 0 engelske rester, søk gir treff, CLS 0,0000/0,0003, 0 sub-ressurs-404.

**8 · URL-redirect `/turguiden` — BLOKKERT.** `urlRedirectCreate` svarer `ACCESS_DENIED`: krever `write_online_store_navigation`, som den lagrede app-auth-en mangler. **Sondre må opprette den i Admin → Online Store → Navigation → URL redirects:** `/turguiden` → `https://turguiden.minhundpet.no/turer?utm_source=minhundpet&utm_medium=redirect&utm_campaign=turguiden&utm_content=redirect`.

**Nye gotchas:** #123 (`/konto` 307 pakker UTM inn i `next=`), #124 (`<style>` i `<head>` taper kaskaden mot `custom.css` — linje 180 mot 751), #125 (`Shopify.analytics.publish` kan ikke spioneres på ved `domcontentloaded`). Alle guards 0 avvik. `last_updated` ikke rørt på noen artikkel — CTA er chrome.)

## STATUS — 2026-09-21 natt (oppgave 9 — §-henvisninger og llms-txt-indeksering) (**LIVE, to commiter + docs.**

**B17 — interne §-henvisninger (commit `032cfa9`).** «se §8», «utdypes i §2», «mer om dette i §10» er raseguide-protokollens seksjonsnummerering, ikke noe leseren noen gang har sett. Nummeret er dessuten bevegelig: settes én H2 inn, peker hver henvisning under den ett hakk feil — stumt, fordi ingenting lenker noe sted. Fant 167 `§` i temaet, klassifiserte hver enkelt, og skrev om **137 forekomster på 34 sider** (33 raseguider + 3 hundetips + boxer-selvreferansen). Hver ble en ankerlenke til riktig seksjon — «se <a href=#helse>avsnittet om helse</a>» — eller fjernet der setningen står fint uten (f.eks. «Utover BOAS (§8) er det tre …» → «Utover BOAS er det tre …»). I FAQ-svar og deres JSON-LD-tvillinger står henvisningen i klartekst uten lenke, slik at HTML og schema holder 1:1.

**Lovhenvisninger er urørt** — det var den viktigste avgrensningen. 68 `§`-linjer står igjen i temaet, alle ekte kildehenvisninger: hundeloven § 6/§ 9/§ 10/§ 13/§ 14/§ 15/§ 28, dyrevelferdsloven § 9/§ 11/§23/§ 25/§ 26/§ 23–24, veitrafikkloven §3, forsikringsavtaleloven § 4-9, forskrift om hunder § 1/§ 2, kommunale hundeforskrifter, grønlandsk Landstingslov §§ 3 og 19, markedsføringsloven §§6–8 og Norsk Rottweilerklubbs avlskriterier §7 (klubbvedtekt, ikke seksjonsnummer). To rettinger på kjøpet: pudels «§10» om doodle-posisjonen pekte på feil seksjon og lenker nå til doodle-seksjonen, og boxers «(se §1-fakta)» var en selvreferanse inne i §1 og er borte.

**B8 — /pages/llms-txt ut av indeksen (commit `f87883e`).** Siden er en nettleser-visning av `/llms.txt`, altså hele kunnskapsbasen med 149 sidelenker duplisert som HTML, og den lå i `sitemap_pages_1.xml` sammen med de 152 ekte sidene. To grep: `<meta name="robots" content="noindex, follow">` i `templates/page.llms-txt.liquid`, og metafeltet `seo.hidden = 1` på side `120709775438` via Admin API. Førstevarianten la taggen øverst i seksjonsfila — live-render viste at Shopify pakker enhver seksjon i `<div id="shopify-section-page-llms-txt">`, så taggen havnet i `<body>` der Google ignorerer den. Flyttet ut i templaten, mellom «layout none» og «section», er den første node i dokumentet. `/llms.txt` og `/llms-full.txt` er ikke rørt: text/markdown, ikke i sitemapet, og en meta-tagg der ville bare vært støy i fila.

**Vakt:** `scripts/jargon-guard.py` stopper nå «§» + siffer i synlig tekst, og slipper gjennom lovhenvisninger. Diskriminanten er form + nærkontekst: intern henvisning er «§» umiddelbart etterfulgt av siffer uten lov-/forskrift-/klubbord innen 100 tegn foran. Vinduet er 100 tegn og ikke hele linja med vilje — linjene er ofte 400–1500 tegn, og et lovord i starten av avsnittet ville ellers hvitliste en intern henvisning lenger ute. Liquid-kommentarer, `<script>` og `<style>` maskeres bort; derfor står `§Pelsstell` i kommentarene i `mh-product-callout` og `mh-track-product-callouts` igjen — de er ikke synlig tekst.

**Verifisert:** 0 interne §-henvisninger igjen i synlig tekst. 123 nye ankerlenker, 0 døde ankre (1937 ankre sjekket i hele temaet). 0 nøstede eller ubalanserte `<a>`; all JSON-LD i de 34 filene parser; FAQ↔JSON-LD 1:1 på de fem filene med FAQ-endring. Live-pull mot repo: 35/35 filer identiske. `/pages/llms-txt` har taggen som første node, innholdet uendret (149 sidelenker, 88 878 bytes); sitemapet er 153 → 152 URL-er, 0 treff på llms. Stikkprøve med Playwright på fransk-bulldog, berner-sennenhund, hund-vil-ikke-ga-tur og llms-txt: 0 nye konsollfeil (de to som finnes — shop.app CSP-frame og 403 — ligger også på urørte sider som `/pages/hundetips`), og et klikk på første «avsnittet om …»-lenke traff eksisterende mål på alle tre.

**Ikke gjort:** `last_updated` er ikke bumpet på noen av de 34 sidene (rent lenkesveip, ikke redaksjonelt — `FRESHNESS_SKIP=1`). `article_map` er ikke rørt: ingen H2, produktnavn eller tall endret, så llms-Trigger E slår ikke inn. Ingen GSC-reindeksering bedt om — innholdet er identisk, bare navigasjonen er endret.)

## STATUS — 2026-09-21 sen kveld (oppgave 8 — sjargong og SEO-titler) (**LIVE, tre commiter + docs.** Én runde over tre flater som hang sammen. **Del A — synlig tekst:** «moat», «TRIPPEL», «helse-pillarer» og «storrase-helse-trilogi» er ord fra raseguide-protokollen, ikke fra hundeverdenen; 80 forekomster i 31 seksjonsfiler skrevet om til naturlig norsk, variert etter setningen. 24 × «Definerende helse-moat:»-punktet i Kort fortalt ble «Viktigste helserisiko» / «Rasens tyngste helseutfordringer» / «Viktigste arvelige sykdommer» / «Helse å DNA-teste for» / «Helse i avlen» (jamthund, som ikke har noen raseeksklusiv test). 9 × «TRIPPEL raseeksklusiv helse-profil» → «tre raseeksklusive helsetilstander». 11 H2-er omskrevet, deriblant «fire-moat-realiteten» → «de fire tunge områdene» og «autoimmune-moat» → «de autoimmune sykdommene». Sidesaker samme runde: dachshunds TOC-lenke het «IVDD og chondrodystrofi (MOAT)», pudel hadde «markedsføres på hypoallergen- og low-shedding-claims», rottweiler viste til «§8». Ingen fakta, tall, kilder eller sykdomsnavn rørt. `article_map` sveipet i samme commit (Trigger E): 11 H2-navn oppdatert, 145 entries, 0 malformed. **Del B — ankre:** ID-ene bar protokollordet (`#helse-moat`, `#historikk-moat`, `#mdr1-moat`, `#moat-utrydningstruet`) og står i adressefeltet + indekseres som egne URL-er i GSC (gotcha #28). 48 ID-er og de 48 tilhørende TOC-href-ene omdøpt til norske, beskrivende navn (`#helse`, `#degenerativ-myelopati`, `#autoimmune-sykdommer`, `#kritisk-truet`) etter kollisjonssjekk mot eksisterende ID-er. Sveip over alle 153 sidemaler: **0 døde ankre**. **Del C — titler og schema:** «komplett raseguide» lå i SEO-tittelen på 33 sider, og `Article.description` arvet fra `page.title` på 74 sider fordi `page.content` er tom på to-fil-artiklene (gotcha #122). 63 titler skrevet via `metafieldsSet` i tre batcher (0 userErrors, eksisterende metafelt-type gjenbrukt per side): 33 raseguider, 23 titler som lå 61–74 tegn, 7 sider som manglet `title_tag` helt. Alle 153 sidetitler er nå ≤60 tegn (lengste 59), unike, uten «komplett raseguide» eller «alt du trenger å vite». Tallene er hentet fra guidenes egen verifiserte tekst («Border Collie — 2+ timer aktivitet hver dag», «Sibirsk Husky — 16–27 kg og mye pels»), aldri utenfra. `mh-article-schema` henter nå `page_description` → `page.content` → `page.title`. B6 lukket: `slikkematte-hund` fikk `seo.title`, med `seo.description` sendt ordrett med (gotcha #86). **To feil funnet og rettet underveis:** JSON-LD-en viste `Addison\u0026#39;s` — `json`-filteret escaper i to pass, så både `replace` før og etter `json` på `&#39;` er virkningsløst (gotcha #121); riktig fiks er `replace: '\u0026#39;'` etter `json`. Verifisert live: `Article.description` er nå bit-identisk med `<meta name="description">` på alle stikkprøver. **Vakt:** `scripts/jargon-guard.py` (pre-commit, `JARGON_SKIP=1`) stopper «moat», «TRIPPEL» og «helse-pillar» hvor som helst i temaet og «komplett raseguide»/«alt du trenger å vite» i SEO-flater; `<h1>` er unntatt med vilje. Negativtest slår ut, 628 filer skannet rent. **Verifisert:** 32 live-sider hentet serielt (≥3,2 s, bot-vernet i gotcha om 429) — 0 sjargong i HTML, 0 døde ankre, 1 BreadcrumbList per side, 0 JSON-LD-parsefeil, alle titler ≤60; 6 omdøpte ankre klikk-/hash-testet i Playwright (mål finnes, siden scroller, TOC-lenke peker riktig); konsoll ren bortsett fra Shopifys egen `X-Frame-Options`-advarsel. Sidebar-guard 0 avvik, breadcrumb-guard 0 avvik, `turguiden_cta_click` uendret i alle fem filer. `last_updated` bevisst ikke bumpet (FRESHNESS_SKIP=1) — språkrydding uten faktaendring, samme begrunnelse som d3c58eb. Rollback: `docs/ROLLBACK-seo-2026-09-21.json`. **Ikke gjort, med begrunnelse:** H1-ene «Rase — komplett raseguide» står på 75 guider — synlig sidetittel er en redaksjonell avgjørelse, ikke en SEO-tag. 213 §-referanser i synlig tekst er blandet lovhenvisning og vår egen kapittelnummerering og krever per-setning-vurdering — ny rad B17 i kartleggingen.)

## STATUS — 2026-09-21 kveld (oppgave 6d — Turguiden på forsiden) (**LIVE.** Undersøkelse først (rapport, stopp, godkjenning): forsiden er åtte seksjoner i `templates/index.json` — hero+produktgrid (`custom_liquid_XLGbTn`, 34 165 tegn), trust-bar (`trust-bar-4col`, eneste med `settings: []` og dermed eneste innhold editoren ikke kan endre), Judge.me-karusell (`_blocks`), «Hundetips fra King» (`custom_liquid_tipsPromo`, 4 kort), «Møt King» (`custom_liquid_kingPromo`), nyhetsbrev (`section`), divider, FAQ (`custom_liquid_XRexTm`). **Null Turguiden på forsiden fra før** — de tre lenkene i rendret HTML er meny ×2 (`utm_medium=menu`) og footer (6b), alt navigasjon. Kartleggingens gamle forslag (bytt potevask-kortet, eller banner mellom King og nyhetsbrev) forkastet: kortbyttet ofrer et produktkort og legger Turguiden i en editor-redigerbar JSON-streng, banneren setter varsel-påmelding rett over nyhetsbrev-påmelding. **Valgt plassering:** rett etter «Hundetips fra King», før «Møt King» — under hele handelsblokken (hero → grid → trust-bar → Judge.me), naboen er tips-blokkens eget fullbredde-CTA-kort, og King-seksjonen skiller varsel fra nyhetsbrev. **Format:** stripe, ikke tekstlinje som på PDP — forsiden har ingen brødtekst, og footer-lenken dekker allerede tekstlenke-nivået. **Bygget:** `sections/turguiden-home-strip.liquid` (schema `"Turguiden-stripe forside"`, 24 tegn, `"settings": []`) som rendrer `turguiden-varsel-cta` med `article: 'forside'`, `medium: 'home'`, `position: 'home'`, `layout: 'strip'`. Snippeten fikk to nye parametre, begge med default: `medium` (default `'article'`) og `layout` (default `'box'`, `'strip'` gir klassen `mh-tgcta--strip` + en `<style>`-blokk som **bare** rendres i strip-modus, så boks-sidenes HTML er uendret). `index.json` fikk kun `"turguiden_home_strip": {"type":"turguiden-home-strip","settings":{}}` + plassen i `order` — +92 byte, ingen eksisterende seksjon rørt (verifisert med kommentar-strippet parse mot HEAD). Live `index.json` ble pullet før endringen: semantisk identisk med repoet, ingen editor-drift. **Vakt:** guard-sjekk 7 (order-posisjon og -antall, type, `"settings": []`, schema-navn ≤25, `medium/article/layout/position`-argumentene, ingen «turguiden» i noen `custom_liquid`-streng, totalt én Turguiden-lenke på forsiden) + unntak i sjekk 1 for seksjonen, som ikke har noen `page.*.json`. Fem negativtester slår ut: stripa flyttet i `order`, «turguiden» i `custom_liquid`, duplikat i `order`, schema åpnet for editoren, `medium: 'home'` fjernet. Guard: 93 A/B · 16 boks · 145 kroker · 77 sidebar · 3 produktmaler · 1 forside-lenke · 0 avvik. **Regresjonssjekk (krav fra Sondre):** href + alle `data-*` hentet live FØR push på 3 boks-sider (`bandtvang-norge` med to bokser, `golden-retriever`, `labrador-retriever`) og 3 sidebar-sider (`hund-spiser-gress`, `cane-corso`, `chihuahua`), 14 elementer — sammenlignet etter push: **0 avvik**. **Verifisert:** push kun med `--only` (aldri full push), `theme pull` + diff identisk på alle tre filer (gotcha #112). Playwright 390/1000/1280 px: href eksakt, `scrollWidth` = viewport overalt, hero (145/164), produktgrid (1 110/1 056), første produktrad (1 110–1 438 / 1 056–1 508) og trust-bar (2 135/2 008) **bit for bit uendret**. Stripa: topp 4 262 / 3 446 / 3 582, seksjonshøyde 319 / 226 / 194, boks 255 / 154 / 122 px, stablet på 390 og horisontal (grid) fra 768 px. Alt under flyttes ned med seksjonshøyden («Møt King» 4 262 → 4 581 og 3 582 → 3 776). **Én feil funnet og rettet i samme økt:** første push regnet gutteren to ganger (`max-width: 1200` + 88 px padding) og la stripa 40 px for langt inn på desktop — rettet til to ledd (ytre `--page-margin` 16/40, indre max-width 1200 + 20/48), som treffer `.mht2-grid` nøyaktig på alle tre bredder, ikke bare 1280. Docs: runde 9 i UTM-standarden. **Åpent:** GA4-lytter for `turguiden_cta_click` (alle plasseringer nå: boks, sidebar, footer, product, home) og Vercel-avlesing av `utm_medium=home` medio oktober.)

## STATUS — 2026-09-21 ettermiddag (oppgave 6c — Turguiden på produktsider) (**LIVE.** Undersøkelse først (rapport, stopp, godkjenning): alle 22 `product-*.liquid` listet med Admin-status (16 aktive, 2 arkiverte, 5 draft) og JA/NEI på tur-kobling — JA for TurPakken, Potevasker, Ullgenser (+ valpepakken som svakest; **Sondre strøk den**), NEI for resten (innendørs bruk, tekstil, arkivert/draft; vannskål og poteklipper nedgradert fra kartleggingens «svak» til NEI). Funn: `snippets/mh-buybox.liquid` finnes ikke — kjøpsboksen er inline per mal (`.<prefiks>-atc-btn`), sticky-baren er `position: fixed` 66 px nederst på mobil og `display: none` på desktop; `mh-product-readmore` rendres ikke på vannflaske/potevasker selv om grenene finnes (separat sak, ikke rørt). **Bygget:** `snippets/turguiden-product-line.liquid` — én `<p>` i brødtekst 16 px, lenke `#2d6a35` understreket, ingen boks/knapp; tekst per handle i `case`-gren, ukjent handle rendrer ingenting; URL via `turguiden-link` (`/turer`, `medium: 'product'`, `content: <handle>`, `anchor: 'fylker'`), kommune gjettes aldri; `data-mh-event="turguiden_cta_click"`, `-position="product"`, `-placement="turguiden-product-line"`, `-product=<handle>`. Kroken `{% render 'turguiden-product-line', handle: product.handle %}` står KUN i `product-vannflaske`, `product-potevasker`, `product-ullgenser`, rett etter `</div>` på `.<prefiks>-accordions`, før «Andre produkter du vil like». Tekster (Sondres): «Flasken er pakket. [Finn neste tur på Turguiden], og få varsel når båndtvangen starter der du bor.» / «Gjørmete poter kommer fra gode turer. [Finn neste tur på Turguiden], …» / «Laget for kalde turer. [Finn turer nær deg på Turguiden], …». **Vakt:** guard-sjekk 6 — maks én Turguiden-lenke per `sections/product-*.liquid`, kroken kun i de tre (og i alle tre), snippet-grener = de tre handlene; negativ test (krok i vannskal + dobbel lenke i potevasker) ga 2 avvik, revert 0. **Verifisert:** push av 4 filer, `theme pull` + diff identisk (gotcha #112); Playwright 390/1280 × 3 sider: 1 linje per side, tekst og href eksakt, `rgb(45,106,53)` underline 16 px, scrollWidth = viewport. Linjas topp: 390 px 2 594 / 2 633 / 2 832 (kjøpsknapp 1 108 / 1 263 / 1 352), 1 280 px 1 924 / 2 071 / 2 085 (kjøpsknapp 646 / 801 / 890). Kjent JS-feil B10 (app-pixel `Invalid URL`) uendret, ikke vår. Commit `d057d9e` + docs-commit. Docs: UTM-standard «Runde 8», product-templates strukturnote. **Åpent:** GA4-lytter for `turguiden_cta_click` (alle plasseringer) og Vercel-avlesing av `utm_medium=product` medio oktober; readmore-rendering på vannflaske/potevasker.)

## STATUS — 2026-09-21 formiddag (oppgave 6b + sticky-sjekk) (**Turguiden i footer LIVE.** Undersøkelse først: `sections/footer-group.json` har én seksjon uten settings, `sections/footer.liquid` (543 linjer) er helt hardkodet med `blocks: []`/`settings: []` — theme editor kan ikke røre den, ingen Admin-meny er koblet, `footer-utilities.liquid` er ubrukt, og det fantes ingen Turguiden-lenke der. Minste inngrep valgt: ett `<li>` i kolonne 3 «Hjelp & info» rett under «Hundetips», `render 'turguiden-link'` med `path: '/turer'`, `medium: 'footer'`, `content: 'footer'`, `anchor: 'fylker'`; `data-mh-event="turguiden_cta_click"`, `data-mh-position="footer"`, `data-mh-placement="turguiden-footer"`; lenketekst «Turguiden – varsel om båndtvang» (Sondre), ingen ny CSS. Regnes som navigasjon (som menylenken), ikke boks/inline. **Vakt:** `scripts/turguiden-sidebar-guard.py` sjekk 5 feiler hvis footer.liquid mangler render-kallet (0 avvik i dag). Push av to filer, `theme pull` + diff = identisk (gotcha #112). **Målt live:** href korrekt på begge bredder; 1280 px: kolonnen 219 px, teksten på 2 linjer (47 px); 390 px: 1 linje, ligger under «Produkter» og over nyhetsbrevet. **Sticky-sjekk etter #111 (kun rapport):** 13 unike sticky-regler utenom artikkel-sidebar; 8 live (header 81/103 px top −1, /cart-summary `top: var(--header-height)`, drawer-header/-summary, søkemodal-header, popup-affordanse, pelstype-veiviser-sidebar, oppskrifts-TOC) — ingen overlapper header eller dekker innhold på 390/1280; 5 rendres ikke live (Horizon `product-information`-sticky brukes av ingen PDP, facets/quick-add finnes ikke på /collections/all, `mh-hundedagen` og `quick-order-list` er ikke i noen template). Drawer-summary er statisk på desktop fordi Horizon-JS setter `cart-summary-sticky=false` ved 51 % — checkout-knapp 28 px under folden med 2 varer, stock-oppførsel, ikke rørt. **Fiks utenfor scope, godkjent av Sondre:** `hund-smell` var eneste artikkel med `top: 100px` og `align-items: start` på grid-en → `aside` = sticky-høyde → festet aldri (gotcha #113). Nå 120 px og default align; målt 1280: kolonnen på 120 ved 1500/4000 px scroll, CTA 120–294 + TOC synlige. **Hovedmenyen:** allerede på standarden live (`utm_medium=menu&utm_campaign=turguiden&utm_content=hovedmeny`, 2 forekomster) — docs sa «gammel form», rettet; ingen Friday-instruks nødvendig. Commit `55faa19` + docs-commit. **Åpent:** GA4-lytter for `turguiden_cta_click` (alle plasseringer) og Vercel-avlesing av `utm_content=footer` medio oktober.)

## STATUS — 2026-09-20 natt (oppgave 6a) (**Turguiden sidebar-CTA LIVE på 77 A/B-artikkelsider.** Steg 1 var undersøkelse: `templates/article.json` finnes ikke (ingen blogg) — alle 146 artikler er `page.*.json` → hardkodet seksjonsfil, og 145 av dem har allerede `mh-article__sidebar` med sticky TOC + Tips fra King (eneste uten: `arvelige-sykdommer-hund`, C). Målt live 1280×800: sticky-kolonnen er 790–880 px mot ~680 px synlig bånd, så alt under TOC-en er usynlig mens den er festet (gotcha #111) → CTA-en ligger **øverst, over TOC**, holdt på **174 px**. Mobil (<1000 px): sidebaren stables nederst etter recap-grid/«Les også», rett før King-kortet — brødteksten flyttes 0 px; ingen mobilkopi («én boks per side» i DOM). **Styring:** `page.content` er tomt på disse sidene og Liquid kan ikke lese egen malkilde, så deteksjon av inline boks skjer via liste: `snippets/turguiden-sidebar-cta.liquid` bærer `tg_ab_handles` (93) + `tg_box_handles` (16), tar `handle: page.handle`, og rendres fra kroken som første linje i `mh-article__sidebar-sticky` i alle 145 filer (sed-sveip, `FRESHNESS_SKIP=1` — chrome, ikke redaksjonelt). `scripts/turguiden-sidebar-guard.py` sjekker boks-lista mot `grep -l "render 'turguiden-varsel-cta'"`, at alle A/B-handles har template + krok, og 0 håndskrevne Turguiden-URL-er: **93 A/B · 16 boks · 145 kroker · 77 får CTA · 0 avvik.** Ikke metafelt (Admin på 93 sider, token uten `read_content`), ikke tagger (sider har ingen). Lenke via `turguiden-link`: `/turer#fylker` på alle 77 (ingen kommune kjent, aldri gjettet), `utm_medium=article`, `utm_content=<handle>-sidebar`; `data-mh-event="turguiden_cta_click"` + `data-mh-position="sidebar"`. **GA4-lytteren `mh-track-product-callouts` er IKKE rørt** — parkert til avlesingen medio oktober (gjelder også de 16 boksene). Knapp: gradient #2d6a35 → #3a7d44. Pilot-push (`hundetips-vinter` = `hund-om-vinteren`) avslørte gotcha #112: `theme push --only` på en fil som ikke finnes gir exit 0 og pusher ingenting. Full push: 145 seksjoner + snippet, `theme pull` + diff mot HEAD = 0 avvik; live-sjekk serielt: `hund-spiser-gress` (B) CTA=1, `hund-tisser-inne` (C) 0, `golden-retriever` (boks) sidebar 0/boks 1, `hund-smell` (320 px-variant) CTA=1, 0 Liquid-feil. Docs: vedlegg B (full A/B/C-tabell, 146 rader) i `turguiden-pa-minhundpet-kartlegging.md`, runde 6 i `turguiden-utm-standard.md`, sidebar-komponent 0 i `page-patterns.md`, gotcha #111/#112. Nye raseguider arver kroken fra cane-corso. **Funn underveis (gotcha #111): sidebaren har IKKE vært sticky siden redesignet 09.04.2026** — `html, body { overflow-x: hidden }` i `layout/theme.liquid` gjør body til scroll-container og dreper `position: sticky` sitewide (161 filer). Verifisert i nettleser: `overflow-x: clip` på begge gir `stickyTop` 120 og ingen horisontal scroll. **Fikset og LIVE 21.09** (`overflow-x: hidden; overflow-x: clip;` i den rekkefølgen, Safari < 16 beholder hidden). Verifisert etter push på 390 + 1280 px: forside, potevasker-PDP, drawer åpen, /cart, golden-retriever, hund-spiser-gress — ingen horisontal scroll; sidebar-kolonnen fester seg på 120 px med CTA + TOC synlige samtidig. Sticky virker sitewide igjen (161 filer). **Åpent:** avlesing i Vercel (`utm_content` med `-sidebar`) medio oktober; GA4-lytter da.)

## STATUS — 2026-09-20 sen kveld (**Turguiden: nød-ID-kort for hund bygd ferdig på gren `nod-id-kort` (`626638d`, pushet til origin) — IKKE merget, IKKE live.** Formål: hunden blir borte, en fremmed skanner et kort på selen og ringer eieren. Kortet er for FINNEREN og viser bare navn, bilde, rase, eierens telefonnummer og chipnummer. **Tre gater før merge, alle Sondres:** (1) Google godkjenner Android-appen; (2) App Privacy (Apple: Contact Info → Phone Number, linked) og Data safety (Google: Personal info → Phone number) oppdateres — første telefonnummer Turguiden lagrer, og appen viser nettsiden i WebView, så et telefonfelt live før skjemaene er oppdatert er avvisningsgrunn; svarark i `docs/butikkskjema.md` → «Venter»; (3) migrasjonen `supabase/migrations/20260920190000_hund_nodkort.sql` limes inn i SQL Editor (lagt på utklippstavla 20.09). **Slik virker det:** tabell `hund_nodkort` (hund_id PK → `hunder` cascade, så kontosletting/«fjern hunden»/demo-nullstilling rydder uten at slettekoden er rørt; kode unik 12 tegn fra 32-alfabet uten 0/O/1/I = 60 bits, lages med crypto i appen; aktiv; telefon `+47[2-9]\d{7}`; chip 15 siffer; RLS via hunder som påminnelsene; anon ingen grants; eieren kan ikke UPDATE kode → «Lag ny kode» = delete+insert). Finnerens side `/h/<kode>` leser med service_role og `aktiv = true` i spørringen, så av og ukjent kode er samme 404 («Dette kortet er ikke i bruk», tips om chip/Dyre-ID). Tre lag mot indeksering: noindex i metadata, `/h/` i robots.txt, `X-Robots-Tag` + `no-store` i next.config (målt lokalt: 404 + begge headere). 30 oppslag/IP/time, ingen logging av oppslag. Utenfor /konto (middleware urørt) og utenfor `APP_LENKE_STIER` (AASA/assetlinks urørt). **Eierflyt** på `/konto/hunder/[id]` → bolk «Nød-ID-kort»: lagre telefon (validert som NORSK nummer — alle skrivemåter → `+47XXXXXXXX`, utenlandske/tullenumre avvises) + chip → kortet er AV → forhåndsvisning med SAMME komponent som finneren får (`nodkort-visning.tsx`) + liste over hva som blir synlig → «Slå på kortet». Deretter: skriv ut, åpne finnerens side, endre, slå av, lag ny kode, slett (avkryssing). Statuslinje på hundekortet på Min side. **Utskrift** `/konto/hunder/[id]/nodkort`: A4 med kortet ×2 à 85,6 × 54 mm, QR 40 mm som SVG (`qrcode` 1.5.4, feilkorreksjon Q), svart/hvitt, adresse i klartekst brutt ved `/h/`, lange navn på to linjer; **telefonnummeret står ikke på kortet** (besluttet av Sondre — ellers virker ikke av-bryteren); verifisert som PDF via Playwright print-media. Fra appen: «skriv ut fra nettleser på PC» (WebView-print ikke rørt). Eksporten tar med kortet. Personvern («Kort fortalt» åtte ting; nye avsnitt om nød-ID i «Hundene dine og varsler», «Hvor lenge vi lagrer», «Hvem vi deler med»; rapport-setningen presisert) og vilkår («Brukerkonto») datert 2026-09-20. **Verifisert:** tsc 0 feil, eslint rent på berørte filer, 770/770 enhetstester (ny `nodkort-regler.test.ts` + eksporttest), `next build` grønt. **Ikke verifisert (krever migrasjonen):** hele eierflyten i nettleser og integrasjonstesten `tester/integrasjon/nodkort.test.ts` (anon ser ingenting, av = null, skranker 23514, cascade ved deleteUser) — oppskrift i `docs/konto.md` → «Nød-ID-kort». Én bug fanget av egen test underveis: tolkeren «rettet» 0→O/1→I, men alfabetet har ingen av dem — fjernet.)

## STATUS — 2026-09-20 natt, runde 5 (**Turguiden UTM-harmonisering live: 13 tekstlenker i 8 seksjoner fikk `-inline`-suffiks — `aktivisere-hund-pa-tur` (1), `bandtvang-norge` (6: ⚠️-avsnittet, «Turguiden» og Oslo/Bergen/Trondheim/Nes i §Kommunal båndtvang), `hund-vil-ikke-ga-tur`, `golden-retriever`, `dansk-svensk-gardshund`, `hund-bader-ute`, `hund-om-sommeren`, `labrador-retriever` (1 hver).** Regex-sveip begrenset til `content:`-feltet i `turguiden-link`-kall; CTA-boksene (`article:`) urørt, ingen tekst/markup/plassering endret (9 linjer +/−). `grep` for tekstlenker uten `-inline` gir 0. Harmoniseringslista i `docs/turguiden-utm-standard.md` avkrysset. **Rest, ikke løst (rapportert, venter på vedtak):** `bandtvang-norge` har to tekstlenker til `/turer` (⚠️ med `#fylker`, «Turguiden» uten anker) som nå deler `…utm_content=bandtvang-norge-inline`, og topp-/bunnboksen deler `…utm_content=bandtvang-norge` — `position` ligger bare i `data-mh-position`. Ikke funnet på et tredje mønster. `last_updated`, H2-er, schema, `article_map` urørt (FRESHNESS_SKIP). **Verifisert før push** (preview 149856485454 via primærdomenet + cookie-jar): live = HEAD på alle åtte, 0 boks↔tekst-kollisjoner på sju sider, båndtvang-siden kun de to kjente, theme check = kun pre-eksisterende HardcodedRoutes. **Neste:** vedta mønster for båndtvang-restene; utvid klikk-lytteren; Admin-flatene; Vercel-avlesing per `utm_content` om ~4 uker.)

## STATUS — 2026-09-20 natt (**Turguiden runde 4 live: varsel-CTA + inline tekstlenke på rad 9–15 — flat-coated-retriever (5 944 visn.), norsk-elghund-gra (5 095), irsk-setter (5 016), newfoundland (4 855), flattmiddel-hund (4 700), engelsk-setter (3 974), strihaaret-vorstehhund (3 646). Topp 15 A-sider er dermed ferdig (rad 1–15, tre commits samme dag).** Samme mønster som runde 2–3. **Ny UTM-regel** i `docs/turguiden-utm-standard.md`: boks `utm_content=<handle>`, tekstlenke `<handle>-inline` — ankeret `#fylker` når ikke serveren, så gordon-setter og beagle (runde 3) hadde to identiske URL-er i Vercel; begge rettet i denne commiten. Valgt `utm_content` og ikke `utm_term` fordi content er parameteren for å skille lenker til samme mål, standarden har fire faste parametre og snippeten ikke støtter term. **Ikke harmonisert (eget sveip):** eldre tekstlenker med bar handle — `bandtvang-norge` (⚠️-avsnittet, samme URL som boksen der), `hund-vil-ikke-ga-tur`, `aktivisere-hund-pa-tur`, og vann-lenkene på golden/gårdshund/bader-ute/sommer/labrador (ulik sti, ingen kollisjon). **Plassering, verifisert mot filene:** flat-coated/newfoundland → etter §Aktivitetsbehov (produktkortet ligger i §Pelsstell foran); elghund → etter §Jakttradisjonen — løshund vs båndhund; irsk/engelsk setter → etter §Aktivitet (kortet i §Pelsstell etter H2 + avsnitt, som runde 2–3); flåttmiddel → etter King-tipset som avslutter §Hvor i Norge er det mest flått? med tekstlenke «turer i ditt fylke» i samme seksjon — første forsøk landet én seksjon for langt (info-boksen matchet ikke body-grensen), rettet på preview før commit; **strihåret vorsteh → AVVIK: etter §Temperament**, fordi §Aktivitet bærer SnackSpin-kortet midt i seksjonen og boksen ellers ville stått to avsnitt etter kortet. Ingen side navngir en kommune (flåttmiddel lister fylker/landsdeler for flåttrisiko, ikke leserens sted) → alle bokser `/turer#fylker`. `last_updated`, H2-er, schema og `article_map` urørt (FRESHNESS_SKIP). **Verifisert før push** (preview 149856485454 via primærdomenet + cookie-jar): live = HEAD på alle ni filer, 1 CTA per side i riktig rekkefølge, boks og tekstlenke med ulik `utm_content` på alle ni, theme check = kun pre-eksisterende HardcodedRoutes. **Neste:** harmoniseringssveip av eldre tekstlenker; utvid klikk-lytteren med `turguiden_cta_click`; Admin-flatene (404-menylenker, ordre-/fraktbekreftelse); avlesing i Vercel etter ~4 uker per `utm_content`.)

## STATUS — 2026-09-20 kveld (**Turguiden runde 3 live (`85f638f`): varsel-CTA + inline tekstlenke på rad 4–8 — hund-bader-ute (8 313 visn.), hund-om-sommeren (6 917), gordon-setter (6 598), beagle (6 503), labrador-retriever (6 304).** Fortsettelse av `bf63677`, samme mønster: én `turguiden-varsel-cta` (`position: 'inline'`, sidetilpasset `text`) + maks én inline tekstlenke via `turguiden-link`, `utm_medium=article`, `utm_content=<handle>`. Jakt-/løshundsidene (gordon, beagle) fikk «båndtvang i din kommune» → `/turer#fylker` i §Aktivitet; vann-/sommersidene fikk «turer ved vann» → `/turer-ved-vann`. Ingen side navngir en kommune (gordon nevner distriktsklubbene Vestland/Innlandet, ikke leserens sted), så alle CTA-er → `/turer#fylker`. **Seksjonsnavnene fra runde 2-tabellen ble verifisert mot filene — to avvik:** bader-ute har ingen «trygge badeplasser»-seksjon og §7-stegs plan bærer TurPakken-kortet → CTA etter §Er det trygt for hunden å bade?, lenke i steg 1 (sjekk vannet); sommer-artikkelens §Aktivitet og turer ender med produktkortet → CTA etter §Sommervann i stedet, lenke i samme seksjon. De tre raseguidene fikk CTA etter §Aktivitet som foreslått (produktkortet i §Pelsstell ligger H2 + answer-blokk lenger ned, samme avstand som gårdshunden i runde 2). `last_updated`, H2-er, schema og `article_map` urørt (FRESHNESS_SKIP). **Tre feller i økta:** zsh ord-splittet ikke `$FILES` (memory-regelen) så første kontrollrunde gikk på tom liste — kjørt om med array; en `%%`-formatfeil ga `{%% render` i gordon-fila og Liquid-feil på preview-push — rettet før commit; python-heredoc uten `coding`-linje feilet på æøå så docs-oppdateringen kom som egen commit etter temacommiten. **Verifisert før push** (preview 149856485454 via primærdomenet + cookie-jar): live = HEAD på alle fem, 1 CTA per side i riktig rekkefølge (trygt→CTA→forskjell, vann→CTA→bil, aktivitet→CTA→pelsstell ×3), href-er eksakt etter standarden, theme check = kun pre-eksisterende HardcodedRoutes. Runde 3-tabell i `docs/turguiden-utm-standard.md`. **Neste:** rad 9–15 (flat-coated, elghund grå, irsk setter, newfoundland, flattmiddel, engelsk setter, strihåret vorsteh) når Sondre sier gå; klikk-lytteren; Admin-flatene.)

## STATUS — 2026-09-20 (**Turguiden runde 2 live: varsel-CTA + inline tekstlenke på topp 3 A-sider etter GSC — golden-retriever, dansk-svensk-gardshund, hund-vil-ikke-ga-tur.** Grunnlag: vedlegg A i `docs/turguiden-pa-minhundpet-kartlegging.md` (39 A-sider) sortert på visninger 19.06–16.09, uten `bandtvang-norge`; topp 15 levert som tabell med nærhet til varsel-kroken, plassering og ankertekst — kun rad 1–3 implementert, rad 4–15 urørt. **Levert:** én `turguiden-varsel-cta` per side med ny `position: 'inline'` (dokumentert i snippet-kommentaren) og sidetilpasset `text`, plassert etter seksjonen som bærer tur-kroken (golden §Aktivitetsnivå, gårdshund §Aktivitet og mosjon, tur-artikkelen §Når hunden bare vil en annen vei); mål `/turer#fylker` fordi ingen av sidene navngir en kommune (Sørlandet er landsdel, ikke gjettet til Agder). Inline tekstlenke «turer ved vann» → `/turer-ved-vann` i §Aktivitet på begge raseguidene; tur-artikkelen beholder den eksisterende Turguiden-lenka (L113) som sin ene. `utm_medium=article` for alle tre (flaten skiller fra `menu`/`email`; siden skilles på `utm_content=<handle>`). `data-mh-event="turguiden_cta_click"` beholdt; lytteren i `mh-track-product-callouts` er fortsatt ikke utvidet. Ingen schema-endring, `last_updated` uendret (FRESHNESS_SKIP, ren CTA-/lenkesveip), `article_map` uendret (ingen H2 rørt). **Verifisert før push** på preview-tema 149856485454 via primærdomenet med cookie-jar (gotcha #51): 1 CTA-aside per side i riktig rekkefølge (aktivitet → CTA → helse/pelsstell; annen-vei → CTA → mental), href-ene eksakt etter standarden, `/turer` og `/turer-ved-vann` 200, theme check kun den pre-eksisterende HardcodedRoutes-advarselen (lik HEAD). Menylenka i Admin bruker allerede ny UTM (`utm_content=hovedmeny`) — sett i rendret HTML. Runde 2-tabellen i `docs/turguiden-utm-standard.md`. **Neste:** rad 4–15 (hund-bader-ute, hund-om-sommeren, gordon-setter, beagle, labrador, flat-coated, elghund grå, irsk setter, newfoundland, flattmiddel, engelsk setter, strihåret vorsteh) når Sondre sier gå; utvid klikk-lytteren; ordre-/fraktbekreftelse i Admin.)

## STATUS — 2026-09-18 kveld (**Turguiden tåler treg base — LIVE 19.09.2026: gren `treg-base` (`a05b09f`) merget til main (`5743456`) etter Sondres gjennomgang av forhåndsvisningen; live verifisert serielt: fylke 200 PRERENDER, tur 200, ukjent tur 404, fire OG-kort PNG, `?tur=` 200, /turer 200.** Punkt 3 fra GSC-kartleggingen 18.09: de tre mønstrene (nytt forsøk ved tidsavbrudd, datacache med try/catch utenfor, logg-og-returner-reserve) samlet i `lib/treg-base.ts` og rullet ut: fylkes-/kommunelistene cachet 1 t med nytt forsøk inni; fylkessiden viser overskrift + faktastripe selv om kommuneliste/lengste turer feiler; turdetalj henter fasiliteter/rapporter/vær/båndtvang med `Promise.allSettled` og viser turen uansett — en rendring uten rapporter caches 60 s i stedet for 1 t (`lib/kort-revalidate.ts`); de fem OG-kortene faller tilbake til logo + navn fra slugen med 5 min cache; forsiden `?tur=` mister bare startturen; `app/error.tsx` + `app/global-error.tsx`. 404 for ukjent tur beholdt; basefeil gir aldri 404. **Tre Next 15-feller målt lokalt på produksjonsbygg med simulert basefeil:** (1) Nexts fetch-dedupe ga det andre forsøket en klone av det første 57014-svaret — fikset med et aldri-utløsende `signal` på Supabase-klienten; (2) `connection()` i en ISR-rute i kjøretid = 500 `DYNAMIC_SERVER_USAGE`, erstattet med `unstable_cache(revalidate: 60)`; (3) feilet ISR-førsterendring viser Nexts innebygde 500-side, ikke error.tsx — kun `pages/500.tsx` overstyrer, bevisst ikke gjort (åpen sak i Turguidens `docs/kjente-feil.md`). Verifisert: curl-sett normal/basen nede/bare rapporter nede/kaldt første kall, Playwright på `/turer` (feilkort + «Prøv igjen» henter turlista), 754 enhetstester (28 nye). Vercel-forhåndsvisning krever SSO uten bypass-hemmelighet, derfor lokalt bygg mot ekte base (kun lesing).)

## STATUS — 2026-09-18 (**Turguiden runde 1 live: UTM-standard, varsel-CTA i bandtvang-norge, DRAFT-lenkesikring — commit `076bfdd`, 15 temafiler sha256-verifisert live**)

**Grunnlag:** `docs/turguiden-pa-minhundpet-kartlegging.md` (17.09) — 146 artikler vurdert (39 A / 54 B / 53 C), `bandtvang-norge` er butikkens største side (53 324 visninger/90 d) og eneste flate med dokumentert «få varsel»-intensjon; helsesjekk med to kritiske funn (404-menylenker «Hvor mye mat?»/«Hvor mye vann?» på alle sider — **fortsatt åpent, Admin → Navigation**; repo foran live på sju seksjoner — lukket i dag).

**Levert (én commit, gren `feat/turguiden-utm-varsel-cta` ff-merget):**
- `snippets/turguiden-link.liquid` — alle Turguiden-URL-er i temaet bygges her: `?utm_source=minhundpet&utm_medium=<article|menu|footer|sidebar|email|product|home>&utm_campaign=turguiden&utm_content=<handle>#anker`. Standard dokumentert i `docs/turguiden-utm-standard.md`. Sju eksisterende lenker lagt om (bandtvang ×5, hund-vil-ikke-ga-tur, aktivisere-hund-pa-tur), ankertekst uendret. **Menylenka er Admin-styrt og har fortsatt gammel `utm_medium=nav` — ny URL står i dokumentet.**
- `snippets/turguiden-varsel-cta.liquid` — hvit boks m/ grønn venstrekant, Playfair-overskrift, grønn pilleknapp «Sett opp varsel for min kommune», «Gratis · Tar 2 minutter». Statisk `<a>`, samme fane, `min-height` 252/212 px (målt 255/215 → shift ≤ 3 px). Lenkemål `/turer#fylker` (artikkelen har ingen kommunevelger; Turguiden har ingen `/kommuner`-side, `#baandtvang` finnes bare på kommunesidene). Rendret etter innledning/faktastripe/disclaimer og etter FAQ. Ny tekstlenke «båndtvang i din kommune» i ⚠️-avsnittet. Sporing: `data-mh-event="turguiden_cta_click"` — **lytteren i `mh-track-product-callouts` er ikke utvidet ennå**; UTM måles på Turguiden-siden (Vercel).
- DRAFT-sikring: ni lenker til `treningsgodbiter-glutenfri`, `boffelhale-hund`, `oksemuskel-hund`, `kaninorer-hund`, `hjortegevir-hund` i sju hundetips-seksjoner rendres kun når `all_products[h] != blank and available`; ellers ren tekst. Samme guard-feil rettet i `mh-product-callout` (gotcha #109).
- llms.txt (tre kilder): «4953 turruter / 330 kommuner / ingen innlogging» → «7 318 turruter / 339 kommuner / konto er valgfri og brukes bare til varsler». `last_updated` ikke endret (ren lenke-/CTA-sveip, FRESHNESS_SKIP-semantikk).

**Verifisert live 18.09:** sha256 live = HEAD på 15/15 filer; `/pages/bandtvang-norge` har 2 CTA-bokser + tekstlenke med riktig UTM; klikk på «turer i Oslo» lander på `…/turer/kommune/oslo?utm_…#baandtvang` med «Båndtvang i Oslo»-overskriften på toppen av viewporten og varselskjemaet rett under; de sju seksjonene har 0 DRAFT-hrefs og 0 tomme hrefs; `/llms.txt` viser 7 318.

**Neste (fra kartleggingens liste A/B):** Admin-runde — rett de to 404-menylenkene og bytt Turguiden-menylenka til ny UTM; ordre-/fraktbekreftelse med Turguiden-blokk; topp-15 artikler med CTA via snippetet (parameter `kommune` for raseguider som kan navngi sted); utvid klikk-lytteren med `turguiden_cta_click`; helsemoat-meta på greyhound/westie; header-logo `alt`; dobbel BreadcrumbList (63 sider).

## STATUS — 2026-09-09 natt (**CTR-analyse av title/meta-sveipet 10.–12.08 levert + to reverseringer live.** Puljene funnet via metafeltenes `updatedAt` (ikke bare git): juli-pilot 03.07/20.07 (3 sider), meta-korreksjoner 06.08 (12 raseguider), **hovedsveip 10.–12.08 (41 sider)**, bandtvang-sesongflipp 21.08. 25 nye sider 18.–26.08 holdt utenfor. Vinduer 28.07–09.08 mot 25.08–06.09 (13 d, samme ukedager, GSC side-dimensjon uten anker-URL-er, siste GSC-dato 06.09). **Sveipet: 0,70 → 0,80 % CTR (347 → 393 klikk på ~49 000 visn), posisjon 8,1 → 8,0. Kontroll (60 urørte artikkelsider): 1,35 → 1,02 %, posisjon 8,1 → 8,0.** Holder i begge segmenter (raseguider 0,66 → 0,72 mot kontroll 0,89 → 0,72; hundetips 0,76 → 0,91 mot 1,46 → 1,19). Med 6-ukers baseline krymper løftet til 0,75 → 0,79 %, kontrollen faller fortsatt (1,49 → 1,19). Netto ~0,3 pp mot kontroll ≈ 45 klikk/13 d — usynlig i trafikken fordi ukevisningene doblet seg (39 700 → 77 300) mens korpus-CTR falt 1,05 → 0,72 % siden juni; kontrollfallet starter brått uke 24.08 med flat posisjon (uforklart, ser ut som SERP-endring). Kun 7 av 41 sider utenfor støy (|z| ≥ 1,9): opp bandtvang, hund-kaster-opp («Når det haster» 2,14 → 3,18), welsh-corgi («ryggen er den svake delen» 0,32 → 1,13), gordon-setter («tyngste av de tre setterne» 0,60 → 1,75), hund-slikker-ansikt; **ned hund-vil-ikke-spise (1,07 → 0,38, triage-vinkel lånt fra kaster-opp)**. Mønster: head-term først + én konkret sjekkbar opplysning vant; adjektiv-karakteristikker («skjør, rask og svært kosete») ga ingenting; negative helsekroker på familieraser trakk alle ned (beagle 0,37 → 0,17, chihuahua 0,82 → 0,48, labrador 0,76 → 0,47, boxer 0,54 → 0,37); bade-hund tapte da head-termen ble skjøvet ut av front; hvor-mye-vann-hund ble forlenget til 67 tegn 12.08 og avkortes igjen (1,90 → 1,37). Topp-10 høyest visn/lavest CTR: 9 av 10 var i sveipet (giftig-mat 0,28 % på 5 370, beagle, whippet, golden, italiensk-mynde, finsk-lapphund [urørt], hva-kan-hund-spise, bandtvang, chihuahua, hund-vil-ikke-spise). **Utført etter Sondres godkjenning (Admin API, 10 metafelt, 0 userErrors, API 10/10, live 5/5 på title + tre description-tagger):** hund-vil-ikke-spise tilbake til før-sveip-verdiene (title «Hund vil ikke spise — årsaker og hva du gjør», meta «Hunden lar maten stå urørt? …»); beagle («9–14 kg og 1–1,5 time tur daglig»), chihuahua («1–3 kg og 30–45 min aktivitet daglig»), labrador-retriever («60–90 min mosjon daglig»), boxer («trenger 1–2 timer aktivitet daglig») — alle fire tester samme hypotese (hverdagstall fra Sankt Bernhard/Newfoundland/Weimaraner-mønsteret), metaene flyttet helsen til andre halvdel. Forrige verdier + dato i `docs/ROLLBACK-seo-2026-09-09.json`. De øvrige 36 sveipe-sidene urørt. **Neste avlesing 22.09** (28 d): samme 13-d-vindu mot kontrollgruppe, ikke mot trafikk. Åpent: 24 titler over 60 tegn (kennelhoste 74, klippe-klor 73, hund-kroppsspraak 71) og 42 «komplett»-titler — hund-alene-hjemme (4 893 visn, 0,80 %) størst; giftig-mat/hva-kan-hund-spise trenger SERP-sjekk før en tredje omskriving.)

## STATUS — 2026-09-09 sent (**Badebørste-PDP: visuell paritet med PawTrim — og fella som skapte inntrykket.** Sondre så `/products/badeborste-hund` med Horizons standardmal (lang H1, antall-velger, innrammede løftebokser, «3 personer ser på dette», «Bestill innen 14:00», «Les mer om produktet», loddrette miniatyrer) mens PawTrim så riktig ut. Live serverte custom-malen hele tiden (curl + Playwright + seks ferske cookie-jarer, `templateSuffix = badeborste` i Admin API); årsaken var en `preview_theme`-cookie fra theme-editoren som viste `deletion-preview` (#149856485454) — det temaet hadde PawTrim-malen (08.09) men ikke `product.badeborste.json` (09.09), så den ene siden falt til `product.json`. Reprodusert med `?preview_theme_id=` i Playwright (bunnlinja «deletion-preview · Draft», «8 personer ser på dette»). Gotcha #106. **Reelle avvik mot PawTrim, rettet og live:** videokortet 16:10 mørkegrønn gradient (var 16:7 beige — sidens største tomme flate), «Slik fungerer»-gridet 2 kolonner med kort 3 i full bredde (var 3 smale), hero-`img` `object-fit: contain` på hvit boks, spec-linje «PawWash · såpedispenser 1 dl · 55 silikonpigger · silikon og plast · 18,6 cm» (Sondres ordlyd). Kjøpsboksen (byline → H1 → spec → avsnitt → pris → fire ikoner → knapp → betalingslogoer) var allerede identisk med PawTrims og er urørt. Sondre re-eksporterte alle fem produktbildene samtidig (fyll 67 % → 30 %, nye media-ID-er, alt-tekster satt på nytt) — derfor ingen CSS-padding oppå bildets egen luft. Mal + seksjon pushet til `deletion-preview` så preview-økter ikke faller til default igjen. STEG 19 på live: FAQ↔JSON-LD 6/6, 3 schema-bilder, InStock, 10 lenker/bilder 200, 0 avvik. **Åpent:** ingressen er 4 linjer mot PawTrims 3 (pris/knapp ~50 px lavere) — innhold, Sondres valg; `ab-golden-preview` mangler fortsatt både poteklipper- og badebørste-malen (rørt ikke før 11.09).)

## STATUS — 2026-09-09 kveld (**Nytt produkt: Badebørste til hund med såpedispenser – PawWash (`badeborste-hund`) — PUBLISERT og live; den gamle sjampobørsten er ARKIVERT og `/products/hundesjampoborste` 301-er i ett hopp til den nye siden.** Produkt-GID `8058873086030`, 249 kr uten compare-at (ny vare), SKU `MH-PWSH-GRONN`, kostpris 70, produkttype Hundepleie, tags, metafelt (`custom.produsent` Dongguan Eversoul, `custom.modell` PawWash, `custom.kort_beskrivelse`), SEO-title 58 tegn + description 145 tegn (alle tre meta-tagger verifisert live), kolleksjoner `pleie` + `populaere-produkter`, publisert til Online Store, `templateSuffix = badeborste` satt i `productCreate`. **SEO-grepet:** H1 «Badebørste til hund» (primærord — «sjampobørste» er på norsk et menneskeprodukt og ga null organisk trafikk); synonymene vaskebørste/dusjbørste/sjampobørste/badebørste for hund vevd inn i ingress, H2-er, FAQ og alt-tekst-planen; sekundær krok røyting/pelsskifte (silikonpiggene drar ut løse hår i badet). Ny PDP `sections/product-badeborste.liquid` + `templates/product.badeborste.json` etter PawTrim-mønsteret: feature-grid, tre forklaringskort (fyll/lukk/trykk · røyteperioden · rengjøring og oppheng med stålkroken), spec-tabell med de ekte målene (18,6 × 7,5 × 8,6 cm, beholder 1 dl, 55 silikonpigger) som også ligger i Product-schemaet som `additionalProperty` (Bredde/Lengde/Høyde/Beholder 100 ml — to nye valgfrie felt i `mh-product-schema`), material «silikon og plast» (juridisk regel: aldri «helt i silikon», aldri FDA/REACH/matgodkjent/giftfri, ingen helsepåstander), 6 FAQ med FAQPage-JSON-LD 1:1 (sjampomengde, tørr pels, valp, vask av børsten, tykk underull, materiale), delt «Meld fra når den er tilbake»-skjema, **eget videokort for King-videoen under galleriet (ikke i karusellen)** — kilden er tom til Sondre limer inn mp4-URL. validator.schema.org: Product 0 feil/0 advarsler; pelsfjerner og poteklipper uendret etter schema-endringen. **Utfasing:** gammel handle frigjort (`hundesjampoborste-utgatt`), nytt produkt opprettet på den gamle handelen og omdøpt til `badeborste-hund` med `redirectNewHandle: true` — Shopifys automatiske redirect, fordi tokenet mangler redirect-scope (ny gotcha). Gammelt produkt ARCHIVED (hadde 1 på lager). Alle tema-referanser pekt om: King-badge-lista, llms-produktlista (14 produkter, verifisert på alle tre URL-er), readmore, `page.bade-hund.json`, og sju artikler (bade-hund, hund-liker-ikke-bading, hund-lukter-vondt, hund-klor-seg, allergi-hos-hund, hund-oeyne, pelsskifte-hund); produktkortene i bade-hund og hund-liker-ikke-bading byttet til PawWash, **nytt `mh-product-callout` i pelsskifte-hund §Bading**; hund-klor-seg mistet «stimulerer blodsirkulasjonen»; hund-lukter-vondt fikk pelsfjerner-ordlyden rettet til kanon (velur-underside). Raseguider fikk ingen kort — ett-produkt-regelen. **Gjenstår for Sondre (rekkefølge):** (1) sett lager 9 i Admin — tokenet mangler `write_inventory`, så siden viser «Utsolgt»/meld-fra og alle produktkort er skjult til da; (2) last opp bilder → alt-tekst settes etter `docs/badeborste-bilder.md`; (3) mp4-URL til King-videoen; (4) King-observasjoner til «Tips fra King» (tom med vilje — ingenting oppdiktet); (5) Rich Results Test — krever Google-innlogging, kunne ikke kjøres headless; (6) pelsfjerner-pakken — **LØST 09.09 sent:** varianten omdøpt «1 stk + badebørste», pris 349/referansepris 428 urørt, bundle-reveal peker på `badeborste-hund` og rendrer igjen, fjerne-hundehår-kortet «Pelsfjerner-hanske + badebørste». Friday satte egne alt-tekster på de fem bildene etterpå (alle fem verifisert). **Tillegg 09.09 sent:** lager 9 satt av Sondre (produktkortene i de tre artiklene er nå synlige), fem bilder lastet opp og alt-tekst satt per motiv via `productUpdateMedia` (synonymene fordelt), King-videoen (20,9 s, uten lyd) koblet i videokortet og avspilling verifisert i WebKit, Product-schema har tre bilde-URL-er og `InStock`. «Tips fra King» fylt med Sondres egen tekst (stod rolig, satte pris på skrubbingen, koselyder, glad etterpå). Rich Results Test fortsatt ikke kjørt herfra — Googles verktøy krever innlogget Google-konto i nettleseren. STEG 19 kjørt mot live-render: tre språkavvik rettet (semikolon i prosa, «underulla», «flaska») og Admin-beskrivelsen omskrevet så den ikke gjentar ingressen ordrett; deretter 0 avvik. Fasit: `docs/products.md` → «Badebørste til hund – PawWash».)

## STATUS — 2026-09-08 kveld (**Nytt produkt: Poteklipper til hund – PawTrim — opprettet som DRAFT, egen PDP-mal live, VENTER PÅ SONDRES TEKSTGODKJENNING før publisering.** Produkt `poteklipper-hund` (GID `8058148028494`): beskrivelse, 399 kr, SKU `MH-PK`, vekt 87 g, produkttype Hundepleie, tags, metafelt `custom.modell`/`custom.produsent`/`custom.kort_beskrivelse`, SEO-title 56 tegn + description 153 tegn, kolleksjoner `pleie` + `populaere-produkter`, `templateSuffix = poteklipper` satt via Admin GraphQL (`store execute --allow-mutations` — ikke lenger et manuelt Admin-steg). Ny PDP `sections/product-poteklipper.liquid` + `templates/product.poteklipper.json` etter SnackSpin-/pelsfjerner-mønsteret: H1 «Poteklipper til hund» (søkeordet, hardkodet), tre salgsargumenter som teknisk forklaring (stillegående; keramikk leder varme dårligere enn stål så knivhodet holder seg kjølig; smalt hode mellom putene), spec-tabell, 5-stegs bruksanvisning, 6 FAQ med FAQPage-JSON-LD 1:1, sikkerhetsinfo med «senk aldri enheten i vann». Product-schema får material/mål/vekt (nytt valgfritt `mh_weight`-felt i `mh-product-schema`). **Utsolgt-løsning:** ingen noindex/avpublisering — ved 0 på lager viser siden «Utsolgt» + et «Meld fra når den er tilbake»-skjema (`{% form 'customer' %}`, tag `restock-poteklipper`, ingen app), schema `OutOfStock`. Verifisert på draft-forhåndsvisningen med lager 0. Readmore-lenker til vaske-hundens-poter, klippe-klor-hund, hund-om-vinteren. Bildeplan med alt-tekster: `docs/poteklipper-bilder.md`. **Gjenstår (manuelt):** lager 9 stk (shopauth mangler `write_inventory`, bevisst), bilder, publisering til Online Store, evt. kostpris; deretter llms-liste og Nyhet-badge som egne valg. **Tillegg samme kveld (godkjent scope, ikke publisert):** tre spesifikasjoner lagt til (LED-lys, LCD i prosent, to hastigheter lav/høy — 6 000 o/min = høy), LED som fjerde salgsargument, lav hastighet ved tilvenning; to lazy videokort under galleriet (LED i mørket, King-pote) med tomme kilder og temaeditor-innstillinger, oppskrift i `docs/poteklipper-bilder.md`; «Tips fra King» først tatt ut, så **rettet samme kveld: Sondre har prøvd den på King** — tipset fylt med kun hans tre observasjoner (ingen tilvenning nødvendig, kiler litt mellom tærne, smalt hode kommer til / tar bra med pels), og `poteklipper-hund` lagt i `mh-king-tested-handles`. Gotcha #97 (schema-header maks 50 tegn, push «fullfører» med feil). **PUBLISERT samme kveld etter godkjenning:** ACTIVE + `publishablePublish` Online Store; live `/products/poteklipper-hund` 200, i products.json, kjøpbar med 9 på lager, Product-schema InStock (material/3×16,5 cm/87 g), FAQPage 6, 5 bilder med alt-tekst, begge videokortene koblet (LED 8,6 s m/ lyd, Kings pote 12,7 s uten lyd — byttet fra 79 MB-fila) og Playwright-verifisert lazy på live; lagt i llms-products-data (14), begge Nyhet-listene (ikke mh_curated → badge kun på kolleksjonssider) og King-lista; indeksering bedt om via request_indexing.py. Judge.me-forespørsler: global app-innstilling, ikke verifiserbar herfra. **Bug etter lansering, rettet innen timen:** miniatyrene byttet ikke hovedbilde — `srcset` fra LCP-migreringen 04.09 overstyrte `src` i galleri-JS som gjenbruker `<img>`; SnackSpin hadde samme feil siden 04.09. Begge rettet og pushet, Playwright på live: alle miniatyrer bytter på 10 PDP-er, desktop + mobil. Gotcha #98.)

## STATUS — 2026-09-08 (**Newsletter-popup skrudd AV på artikler/raseguider/blogg — LIVE (godkjent av Sondre, push + hash-verifisert; live-Playwright: forside/PDP åpner og publiserer `mh_popup_view` med page_type index/product, labrador + hund-kaster-opp rendrer ikke elementet).** Ny schema-checkbox `show_on_content` (default false) + Liquid-vakt etter variantvalget i `sections/newsletter-popup.liquid`: variant B rendres ikke på `template.name` page/article/blog; variant A (forside, PDP, kolleksjon) og all GA4-sporing uendret. Bakgrunn: 0,28 % på innholdsflater mot 4,6 % på PDP/forside. Preview hash-verifisert; Playwright med samtykke primet: forside + PDP åpner (variant A), labrador + hund-kaster-opp rendrer ikke elementet, live-kontrast åpner på begge; ingen JS-feil fra temaet (kun Shop-app CSP + wpm-pixel, pre-eksisterende). Gotcha #96 (popupen fjerner seg selv uten samtykke i headless). Skru på igjen: huk av feltet i temaeditoren, eller `"default": true`. Fra 08.09 er GA4-popup-tallene kun PDP/forside/kolleksjon — les med pixel-kanal-filteret (gotcha #95).)

## STATUS — 2026-09-07 kveld (**Popup-avlesning frys #2 (12.08–07.09) levert — `docs/popup-avlesning-2026-09-07.md`.** Påmeldingsrate **0,84 %** (7/830) = laveste terskel (< 2 %, «strukturelt galt»); steg 1-passering **8,3 %** (69/830) = < 15 % («fiksene traff ikke»). Frys #1 deduplisert: 0,61 % / 3,3 %. **Målefunn som overstyrer rå-tallene: hvert `mh_popup_*`-event ligger dobbelt i GA4** — én kopi fra Custom Pixel `150765646` (`pagePath` = (not set)), én fra en avsender med ekte sesjon; skriptets 1 625/133/14 er 2×. Gotcha #95. **Kryssjekken er ren:** de 7 dedupliserte innsendingene matcher 7 `king-popup`-kunder på minuttet, variant og valg — skjemaet lekker ikke; «14 mot 7» og frys #1s «6 mot 3» er artefaktet. Uke for uke: 5 av 7 påmeldinger i Facebook-uka 34, **0 påmeldinger siste 12 dager (225 view)**. A (kommersielle flater) 4,6 % på 109 view mot B (artikler) 0,28 % på 721 — ingen vinner kåres, men B er dramatisk dårligere; variantvalget er per sidetype, ikke randomisert. page_type: PDP 6,1 % (4/66), forside 3,4 %, artikler 0,28 % (2/703). `alle` = 4,5 % (premisset holder); dismiss 99,7 % `close`; timer 97,9 %; trigger→konvertering ikke målbart (`trigger_type` bare på view). Steg 2 taper 9 av 10 som valgte tema; valp 0 av 23. Alle 7 påmeldinger fra mobil. Koder: `KING15` ×2 (én er popup-kunden 26.08, kjøpte 5 min etter), `KING10` 0, `KINGTIPS15` 0; kodestatus ulesbar (`read_discounts` mangler). Kun lesing — ingen endring i butikk/tema/pixel.)

## STATUS — 2026-09-04 (**Høstsesong-pass på hundehår-/røyte-artiklene.** Fem hår-/pels-artikler kartlagt. **Produktkort: null nye, null dubletter** — alle fem hadde allerede ett `mh-product-callout` fra utrullingen; posisjoner målt til 9,1 % (fjerne-hundehår), 12,8 % (hundehår-i-bilen), 52,3 % (pelsskifte), 65,2 % (vaske-hundeseng, produktet er sengen) og 72,4 % (`beste-pelsfjerner-2026`, 301-et og rendrer ikke). Pelsskifte-kortet ble **ikke** flyttet opp — rule 2 forbyr det; i stedet sporet inline-tekstlenke på 30,9 % (første forankrede tekstil-setning; 20–25 %-vinduet bevisst forbigått fordi alt tidligere er uforankret). **Tittel/meta:** sesongvinkelen ligger i meta-beskrivelsene, titlene fikk kun evergreen røyte-vokabular (aldri «høst» i title — daterer seg om våren). Endret: `pelsskifte-hund` (title+desc), `hvordan-fjerne-hundehar-effektivt-hjemme` (title+desc), `hundehar-i-bilen` (kun desc). `vaske-hundeseng` hoppet over — eneste røyte-forankring står i Kings tips, og rule 7 sier det ikke er en krok. **Sidegevinst:** den tilbaketrukne «dobbeltsidig»/«elektrostatisk mesh»-påstanden sto fortsatt på **åtte flater i fem filer** 16 dager etter PDP-rettingen 19.08 — inkl. to alt-tekster, PDP-FAQ + FAQPage-tvillingen og `llms-products-data` (altså live i `/llms.txt` for AI-crawlere). Alt rettet, kanonisk ordlyd låst i `docs/products.md`, gotcha #93 skrevet. 6 filer live, hash-verifisert; meta verifisert på alle tre taggene. **Neste:** GA4-effektmåling tidligst 28 d etter hver pulje.)

## STATUS — 2026-09-04 natt (**Produktkort-arbeidet ferdig i begge korpus — sluttrapport skrevet.** Raseguider: 74/74 på malen, 61 brødtekst-kort, 71 ett bunn-kort, 3 uten bunn. Hundetips: 53 behandlet, 35 kort, 17 ingen fit, 1 bare bunn. `beste-pelsfjerner-2026` bygget som foreslått (callout i §King, kort 2 ut, CTA omdøpt, 4 → 3 lenker; dobbeltsidig-feil, «leveres gratis» og forbudsrest rettet; kontrollpass PASS, hash-verifisert) — **men siden svarer 301 til fjerne-hundehår siden pelsfjerner-cluster P4**, så endringen rendrer ikke; kartleggingen i pulje 5 overså redirecten. Sluttrapporten (kort per produkt på tvers, 30 ingen fit per årsak, visninger dekket, 20 regler) ligger sist i `docs/produktkort-utrulling.md`. **Neste:** effektmåling i GA4 tidligst 28 dager etter hver pulje (dimensjonene registrert 04.09).)
...
## STATUS — 2026-09-04 natt (**Hundetips fase 2+3 slått sammen — migrering til `mh-product-callout` og avviksrydding i samme sveip; pulje 1 av 5 live.** Sondres begrunnelse: 45 artikler har v1-boks eller productduo (52 i kartleggingen minus fase 1 og smerte-tegn); rydder vi først må vi inn i samme filer igjen, og det gamle formatet er svakere (dobbel ramme, lite bilde, knapp uten flate, «Fra 179,00 kr»). **Plan for alle 45 godkjent** (5 puljer à 9, sortert på visninger) med ni avgjørelser: hvor-mye-mat, valp-tisser-inne, aktivisere-hund-pa-tur = ingen fit; hund-om-sommeren = TurPakken (over-fold-boksen fjernes); graver-i-hagen = Andefanten; reise-utlandet = TurPakken; hund-og-barn = Akt.skål; hund-om-vinteren = Ullgenser (sidebar-kort ut); hold-hunden-din-aktiv CTA → SnackSpin. Regel ved bygging: står kortet inntil en helseadvarsel i fila, bygges det ikke — meldes som ingen fit uansett forhåndsgodkjenning. **Snippeten utvidet bakoverkompatibelt** før første bygg: `article` (→ `data-mh-article`; `data-mh-breed` bare når breed er sendt) og `eyebrow` (default «Anbefalt til dette temaet» for hundetips). **Pulje 1 (177 118 visn/90 d):** migrert hva-kan-hund-spise (CalmBall), hund-sover-mye (Hundeseng), hund-kaster-opp og hund-vil-ikke-spise (Akt.skål), giftig-mat (CalmBall), hund-spiser-gress (SnackSpin), hund-alene-hjemme (CalmBall — Sondres valg; Andefanten var forankret i en advarsel om å rydde den bort, ikke i et behov). Ingen fit: hvor-mye-mat (Sondre) og **valp-biter-pa-alt** (kortet sto inntil «ta kontakt med veterinær» + retinerte melketenner/tannstein — fjernet, ikke bygget; §7-stegs plan steg 1 foreslått som ny plassering). Kontrollpass i fersk subagent: PASS, ett lavt flagg («frosset» i giftig-mat-desc ikke forankret — strøket). Pull-verifisert identisk, live 9/9 + samojedhund-regresjon. Lager: Akt.skål 3 (kort på 39 000 visn), Andefanten 2, Sjampobørste 2 — snippeten skjuler kortet ved utsolgt. **Neste:** pulje 2 (hund-kroppsspraak … hund-spiser-avforing), produktvalg per pulje godkjennes underveis.)

## STATUS — 2026-09-04 natt (**Hundetips touchpoint-opprydding fase 2, pulje A — de seks verste etter den nye regelen.** Regel vedtatt av Sondre: brødtekst-kort = ett produkt, bunn-CTA = samme produkt, tekstlenker flere tillatt men maks tre, intet forankret → flaten fjernes, fjern lenken behold setningen; to tillegg: dubletter (lenke til kortets produkt i samme seksjon) fjernes som i raseguide-malen, forbudsrester rettes til scope. Skillet: kort og CTA er anbefalinger (valgparalyse), en tekstlenke er en referanse — men sju lenker er en katalog. **Kartlegging av alle 71 (kun lesing):** 29 med avvik, 42 uten; 87 tekstlenker; kort ≠ CTA i 9, flere produkter i kort/bunn i 14, uforankret kort-/bunn-produkt i 11, 4+ lenker i 5, 8 uforankrede lenker, dubletter i 18. **Pulje A:** `hund-smerte-tegn` — kort («ikke et tiltak mot smerte»), CTA («For hunder med smerte ved nakke-bøy») og lenke motsa hverandre på en helseartikkel, ingen forankring → alle tre fjernet, null produktflate (6 189 visn/90 d); `hund-og-varmen` kort Akt.skål står, CTA byttet, 5 → 3 lenker, forbudsrest til scope; `hund-i-bil` kort TurPakken står, CTA byttet, 4 → 2 lenker, «Viktig: … aldri på selve hunden» slettet; `hund-liker-ikke-bading` kort Sjampobørste står, CTA byttet, 5 → 3; `ferie-med-hund-i-norge` duo → ett kort **Potevasker (Sondres valg)**, 7 → 3 lenker, forbud strøket; `hund-blor-fra-poten` bunn 3 → 1 (Potevasker), SnackSpin/hundeseng var uforankret. Kontrollpass i fersk subagent fant én reell feil før push (overflødig `</div>` etter det nye ferie-kortet lukket hovedkolonnen for tidlig) — rettet; alt annet PASS. Pull-verifisert identisk, live 6/6 (kort, CTA-href, 0 forbudsrester). Baseline og begrunnelse i `docs/produktkort-utrulling.md` («Hundetips — fase 2, pulje A»). **Åpent:** 23 artikler med avvik gjenstår (pulje B+); ett-produkt-regelen for hundetips gjelder nå på tvers av kort og CTA, mens lenker kan peke på flere.)

## STATUS — 2026-09-04 natt (**Hundetips produktkort-opprydding fase 1 — seks brødtekst-kort fjernet fra helsetunge artikler, ett beholdt.** Forarbeid: kun-lesing-kartlegging av alle 71 hundetips (GSC 90 d 03.06–01.09: 437 523 visninger, median 3 045; ord 1 504–4 337, median 2 973; H2 8–17, median 12 — 55 % har nøyaktig 12). Produktflate i dag: 57 av 71 har en eller annen flate (v1-boks i brødtekst 41, productduo 11, bunn-seksjon 6, bunn-CTA med produkt 27, tekstlenker i 42 artikler / 95 lenker, sidebar 2), 14 har ingen (11 av dem Helse); kortposisjon median 53 % mot raseguidenes 38,6 %; ingen hundetips bruker `mh-product-callout`. Grovt «ingen fit»-anslag 20–28 av 71. Productduo-parene er malstyrt (v2-regel fra 02.07: maks 2), 4 av 8 er kopiert mellom søsterartikler. **Fase 1:** 13 artikler har helseinnhold i 90 %+ av H2-ene; sju hadde brødtekst-kort. Vurdert mot adjacency-kriteriet, avgjort av Sondre: fjernet i `hund-oeyne` (kortet brukte «sjampo er en vanlig kilde til øyeirritasjon» som krok for en sjampobørste — argumenterte mot seg selv), `hund-klor-seg` (atopi + lenketekst «ørebetennelse», basset-mønsteret), `allergi-hos-hund` (fire tilstander + to vet-henvisninger), `kennelhoste-hund` (§Behandling hjemme, botulisme, bekymringsterskler), `hund-darlig-ande` (tannstein/betennelse + vet; kortet solgte skålen som tannstell = effektpåstand; bunn-CTA pekte på SnackSpin uten forankring → fjernet, artikkelen har nå null produktflate), `hund-spiser-for-fort` (magedreining rett før og etter kortet — ansvarsavsnittet var selv helseteksten; Sondre: «4 919 visninger er ikke verdt det»). Beholdt: `hund-kaster-opp` (§Forebygging er ren stellveiledning). Tekstlenker rørt ingen steder; bunn-CTA i klor-seg beholdt (sjampobørste navngitt i brødtekst og FAQ). Diff rene slettinger; kontrollpass i fersk subagent PASS 9/9; pull-verifisert identisk; live 7/7. Baseline og begrunnelse i `docs/produktkort-utrulling.md` («Hundetips — fase 1»). **Åpent:** 27 bunn-CTA-er og resten av korpuset ikke vurdert; ett-produkt-regelen på tvers av tekstlenker er ikke avgjort for hundetips (klor-seg har lenker til både potevasker og sjampobørste).)

## STATUS — 2026-09-04 sen kveld (**Golden migrert til `mh-product-callout` — siste guide; utrullingen komplett 74/74.** Pelsfjerner i §Pelsstell steg 1 på 57,0 % (dypest i korpuset), bunn ett kort med «utenom pelsskiftet»-vinkel, `productsolo`-markup/`all_products`-oppslag/CSS/A/B-kommentar/bunn-tekstlenke fjernet, ingen brødtekst endret. Kontrollpass med fersk subagent: PASS uten avvik på (a)–(h). GSC-baseline 90 d: 8 153 visn / 53 klikk / pos 7,1. **Sondres tillegg — avlesning av A/B-kortet før fjerning:** kortet kom 22.08 (`97adc08`), ikke juni; 10 `product_callout_click`-hendelser = trolig 5 klikk fra 4 brukere, alle 22.–23.08 under en Organic Social-topp på 294 mobilvisninger; 0 klikk på 25 organiske visninger etterpå; kort og bunn-lenke kan ikke skilles. **Funn med konsekvens for sak 4:** `placement_type`, `position`, `breed_page`, `product_handle` er aldri registrert som egendefinerte dimensjoner i GA4 (bare popup-parametrene er) — Data API-et kan bare gi `eventCount` per side. Registrering fyller ikke bakover; hver dag uten er tapt baseline. Gotcha #92. **Lukket samme kveld:** Sondre ga Editor-rolle; `placement_type`, `position`, `breed_page`, `product_handle`, `article_page` registrert via Admin API og verifisert i Data API-metadata; navnene verifisert mot pikselens Measurement Protocol-body med headless testklikk; sanntid viste klikket. Sanntids-API-et støtter ikke `customEvent:` på propertyen — dimensjonsverdier leses fra standardrapporten når intradag-data er prosessert.)

## STATUS — 2026-09-04 kveld (**Ett produkt i bunn gjelder alle 74 — de 12 «ingen fit»-guidene trimmet fra tre kort til ett.** Sondres begrunnelse: «ingen fit» gjaldt brødtekst-kortet inntil helsetekst; bunn-seksjonen ligger etter FAQ, så adjacency gjelder ikke der, valgparalyse-argumentet gjelder like mye, og to standarder er verre enn én. Produkt valgt etter samme forankringsmetode: pelsfjerner der brødteksten navngir hansken eller alt bærer en tekstlenke (mops, engelsk bulldog, schäferhund, ridgeback, fransk bulldog, basset, dalmatiner), SnackSpin der guiden selv konkluderer med problemløsing (malteser, bullterrier), TurPakken for coton («utholdende på tur» — svakeste valg, flagget). **whippet og greyhound: ingen forankret produkt** (feller lite, for store for genseren, generisk/negativ aktiviseringskrok, senga Ø 50/60 cm passer ikke) — tre kort står til Sondre avgjør om bunn-seksjonen fjernes. Live-verifisert 10/10. Regel og tie-breaks festet i `docs/page-patterns.md`. **Westie (sak 2) samme kveld:** bunn trimmet til ett kort med hundesengen (Sondres helsebegrunnede valg fra pulje 3), kortteksten forankret på Favrot-studien og §Praktiske kjøpsråd uten effektpåstand, sporet SnackSpin-tekstlenke avlenket etter bichon/griffon-presedensen (setning beholdt — én linje å sette tilbake om Sondre vil ha lenka). **Sondres tre avgjørelser samme kveld:** whippet + greyhound → bunn-seksjonen fjernet helt; coton → TurPakken fjernet også («utholdende på tur» er rasebeskrivelse, ikke forankret behov); westie-lenka forblir avlenket. Regel festet i page-patterns: ett produkt i bunn er regelen, null er utfallet når ingenting er forankret. **70 av 74 har ett bunn-kort, 3 har ingen bunn-seksjon, ingen har tre.** Neste: golden-migrering i egen økt med STOP-gates (sak 1).)
...
## STATUS — 2026-09-04 (**Produktkort pulje 6 — 7 live; schäferhund og ridgeback = ingen fit, husky bygget.** Live: springer 33,3 %, lagotto 38,1 %, yorkshire 37,7 % (SnackSpin); grønlandshund 45,1 %, leonberger 33,8 %, malamute 35,3 % (pelsfjerner). Engelsk bulldog = ingen fit (hudfolds-dermatitt navngitt i samme blokk; §Aktivitet er BOAS-advarsler). **Avgjort av Sondre:** schäferhund («våteksem» i samme blokk uten overskrift) og ridgeback («hudirritasjon», «tannproblemer … seniorlidelser» rett etter kortet) = ingen fit; husky bygget («klipping kan gi varmestress» er stellveiledning om eierens handling, ikke advarsel om hundens tilstand — presedens). Kontrollørens forslag om H3 foran helseteksten avvist og forbudt i page-patterns: strukturendring for å gjøre plass til et kort er det «kortet tilpasser seg guiden» forbyr. Ny feilklasse fanget av kontrollpasset: fjern lenken, ikke setningen — tre kort mistet brødtekst-ankeret sitt og er rettet. **50 av 74 har kort; 14 gjenstår (pulje 7) + westie/golden; 8 = ingen fit.**)
...
## STATUS — 2026-09-04 natt (**Produktkort pulje 2 live — 10 raseguider, tre produkter.** griffon-petit-brabancon 26,3 % og mellomschnauzer 34,6 % (SnackSpin), italiensk-mynde 38,2 % (ullgenser — kortet oppgir XS–M / opptil 45 cm brystmål som opplysning, fordi genseren mangler rygglengde-tabell), staffordshire 41,9 %, bichon 51,6 %, chihuahua 41,6 %, pomeranian 47,4 %, dansk-svensk 44,8 %, flat-coated 35,3 %, australian-shepherd 42,1 % (pelsfjerner). Sondre: lager vektes ikke lenger i produktvalget; regelen er forankring i guidens tekst, ikke «feller mye» — bichon fikk pelsfjerner på guidens eget avsnitt om hud-flak og enkeltvise hår, og kortet unngår ordet «røyter». Åtte sporede inline-tekstlenker fra CTA-rulleplanen erstattet av kort; bichons SnackSpin-lenke og griffons hundeseng-lenke avlenket (ett produkt per guide). Seks kort dypere enn 40 % uten inline-lenke — ingen tekstil-krok i 16–29 %-sonen. **Kontrollpass (subagent):** 10/10 PASS på produktvalg og teknikk, bichon-setningen PASS. To griffon-flagg avgjort: «22 °C» står i brødteksten (falsk positiv); «tåler hard trening i varmen dårlig» er aktivitetsveiledning, ikke helseadvarsel — dokumentert i `docs/produktkort-utrulling.md`. Tre småretting på bunn-kort. Live verifisert 10/10: kort, ett bunn-kort, FAQ↔schema 8/8, alle lenker/bilder 200 (to interne lenker ga timeout i første pass, 200 ved ny sjekk). GSC-baseline for pulja lagret. 18 av 74 har kort; whippet = ingen fit.)

## STATUS — 2026-09-04 natt (**Produktkort pulje 1 live — 7 raseguider, alle pelsfjerner.** gordon-setter 33,2 %, beagle 43,9 %, labrador-retriever 40,1 %, shiba-inu 38,5 %, irsk-setter 36,7 %, cavalier 43,8 %, newfoundland 34,0 % (kortets posisjon i redaksjonell brødtekst). Produktvalg lest ut av hver guides egen tekst; whippet = ingen fit (feller lite, ullgenser går bare til 45 cm brystmål), griffon og mellomschnauzer utsatt til SnackSpin er restocket (4 på lager mot 39 bunn-CTA-er). Pelsfjerner-lager 983 + 488 verifisert før bygging. Seks av ti kandidater hadde allerede usporede tekstlenker — premissen «null produktflate» var feil; kortet erstattet lenkene. Tre guider med kort dypere enn 40 % fikk ingen inline-lenke fordi 16–29 %-sonen ikke har noen tekstil/hår-setning — teksten ble ikke endret for å lage en krok. **Kontrollpass med fersk kontekst (subagent) før live:** 3 avvik — newfoundland-kortet påsto «sofa, klær og bilseter» som ikke står i brødteksten, irsk setter-bunnen «bilsete» likeså, og newfoundlands plassering ble diskutert (beholdt i §Pelsstell etter røyte-avsnittet; alternativet i §barn står inntil en allergi-advarsel). Alle rettet. Live verifisert: kort 7/7, ett bunn-kort 7/7, FAQ↔schema 8/8 på seks, alle lenker/bilder 200, cache-flip bekreftet. GSC-baseline i `docs/produktkort-utrulling.md`; effekt måles på `placement_type` i GA4 etter 28 dager. Pre-eksisterende: gordon-setter FAQ 5 synlig ≠ schema.)

## STATUS — 2026-09-04 natt (**Produktkort-mal for raseguider godkjent — `snippets/mh-product-callout.liquid`, pilot `samojedhund` live.** 50 av 74 raseguider hadde ingen produktlenke i brødteksten (47,7 % av raseguide-trafikken); samojed var én av dem med 5 208 visninger/90 d. Malen er én komponent: før/etter-bilde hentet fra Admin via `media_id` (kvadratisk 2048×2048, ikke 16:9 som antatt), Judge.me-stjerner dynamisk (4,8 · 5 — skjules ved 0), betinget «Fra» (droppes når prisen bare varierer på antallsaksen), eyebrow «Anbefalt til denne rasen», pilleknapp 44×44+ med hover-løft og `prefers-reduced-motion`, `media_bg`-parameter med lys default. **Fem låste regler** i `docs/page-patterns.md`: ett produkt per guide · kortet i første seksjon der produktet er svaret · inline-lenke 20–25 % kun hvis kortet står dypere enn 40 % · bunn-seksjonen beholdes med ett produkt og annen vinkel · mørk bildebakgrunn kun for mørke produktbilder. Samojed: kort etter første pelsstell-avsnitt (975 / 2762 ord = 35,3 %, ingen inline-lenke), bunn-seksjonen fra tre til ett kort, tre knapper → «Se Pelsfjerner-hansken →» (navngitt form er kanon fra nå; korpuset hadde 13 ordlyder), usporet tekstlenke fra sprint #62 fjernet. Forbudsformuleringen «ikke for bruk på hunden selv» i bunn-kortet var i strid med tilbaketrekkingen i `c258f77` (22.08) og er ute. Sporing verifisert i Playwright: `product_callout_click` med `placement_type: inline-callout` via `Shopify.analytics.publish`. Cache-flip bekreftet med 9 prober. STEG 19 på live: FAQ↔JSON-LD 8/8, alle lenker og bilder 200. Utrulling på de 73 andre er ikke planlagt.)

## STATUS — 2026-09-04 (**Julegave-artikkelen `julegave-til-hund` live og lukket — hundetips-artikkel #71, korpusets første kommersielle gaveguide; julegave-kolleksjonen i hovedmenyen.** Fase 0–4 kjørt på ett døgn. Phase 0 viste at GSC ikke kan dimensjonere temaet (3 visninger på «gave til hund», og nettstedet har ingen målt julesesong), ingen kannibalisering, og en norsk SERP med kategorisider og listicles uten størrelses-/budsjett-deling og uten «det du ikke bør gi». Vinkling: hundeeier til egen hund, med én H2 til giveren. Ny side (ikke ombygging), handle i entall mot kolleksjonens flertall. **Godkjente avvik fra canonical:** 7 gavekort i `mh-article__recommend-card--gift` med dynamisk pris og lager fra `all_products`, og én CTA-knapp til `/collections/julegaver-til-hund` etter budsjettabellen. Tabellen henter pris og fraktfri-status live, så siden kan stå urørt til neste jul (evergreen: ingen årstall, ingen frist). 3 231 ord redaksjonelt, 12 H2, 8 FAQ 1:1, Article + FAQPage + ItemList, hub-kort `card_73`, breadcrumb Aktivitet, `article_map` 3250 verifisert på alle tre llms-URL-er, 8 innlenker. **Verifisert før skriving:** returvilkårene sier 14 dagers angrerett + 30 dagers utvidet returrett på ubrukte varer — «åpent kjøp» ble derfor «returrett på ubrukte varer» i artikkelen; temaet sier fortsatt «30 dagers åpent kjøp» uten forbehold i footer, trust-linje og 40 filer (egen beslutning). King 5 kg / Petit Brabançon konsistent. **Meta:** title «Julegave til hund – 7 gaver hunden faktisk bruker | Min Hund» (60), description «Sju julegaver hunden faktisk bruker, testet av King: slikkeball, godbitball, donut-seng og pelsfjerner. Gratis frakt over 250 kr, sendt fra Norge.» (146), begge satt i samme `metafieldsSet`. **Tre feil fanget underveis:** (1) `.mh-article__body a` (0,1,1) slår `.mh-article__product-btn` (0,1,0) — knapper i brødtekst får grønn tekst på grønn bunn, løst med scoped override, trolig latent i korpuset; (2) build-skriptet ble kjørt på nytt etter håndrettinger og nullstilte dem stille — skriptet er deaktivert; (3) **zsh ord-splitter ikke `$VAR`**, så `theme push --only $FILES` lastet opp ingenting to ganger med grønn suksessmelding — siden sto live som tom side med riktig meta i ~10 minutter før pull-verifiseringen fanget det (gotcha #91). **Claim-retting i korpuset:** `hund-tygger-pa-alt` sa SnackSpin «roer arousal-nivået» → «vipper og triller og gir hunden noe å jobbe med». STEG 19 på live: 0 avvik etter fix-forward (fraktpåstand i §1/§9, «i maskin», sugekopp-flate). Blindkontroll: uavhengig agent med kun URL fant 15 avvik på live-siden (tabell/prisspenn brukte 1-pakningen, «Fra» på énprisprodukter, ItemList-navn bar «Beroligende», FAQ 4 nesten ordrett lik §8-answer, returrett-formulering, to FAQ-lenker manglet, m.m.) — 14 rettet og re-pushet samme natt, 1 beholdt («leverer vi ofte samme dag» på Sørlandet er Sondres eget løfte). Første runde av kontrollen traff en tom side fordi pushen ikke hadde landet — det var slik gotcha #91 ble funnet. GSC: side og kolleksjon sendt inn, `notifyTime: null` (gotcha #31). **Åpent:** hero er SnackSpin-produktbilde som plassholder til Sondres King-bilde; `shopauth` utvidet med `read/write_online_store_navigation` (menuUpdate) og publications-scopes.)

**Samme døgn, natt til 05.09 — dateModified-løpet + AI-synlighet-audit (commits 39118ff, 051b179).** Auditen (`docs/ai-synlighet-audit-0309.md`) fant fem ekte hull: logo-URL 404 i 14 skjemafiler (fil lastet opp 03.09, gotcha #88 — 1024 px, 1200 px-kravet for Article.image gjenstår), død `dateModified` på hele korpuset, GTIN 0/15, SKU mangler på `puslespill-hund`, Bing-sitemap udokumentert. robots.txt blokkerer ingen av GPTBot/OAI-SearchBot/ClaudeBot/PerplexityBot/Google-Extended/Bingbot. **dateModified løst** (`docs/datemodified-analyse.md`, løsning 1): `page.updated_at` finnes ikke i Liquid (gotcha #57), så 109 av 144 artikler hadde schema-dato ≠ synlig dato og 121 av 144 hadde schema eldre enn siste git-commit. Nå er ISO-dato på linje 1 i seksjonsfila **eneste kilde** for både synlig «Sist oppdatert» (ny `mh-date-nb`) og `dateModified` (`mh-article-schema` tar `last_modified`, `about`, `hero_image`, `citations`; `lastReviewed` fjernet). 146 artikler koblet på — alle fikk 2026-09-04 som første verdi (Sondres beslutning: ingen manuell 146-raders liste), tre beholdt nyere dato (halsband 26.08, vinter og valpens-utvikling 21.08). De to inline-artiklene (ferie/reise) byttet til snippeten; julegave fulgte etter i 051b179. **Push-vakt** `scripts/freshness-guard.py` (pre-commit, lokal hook — installering i `docs/workflow.md`) blokkerer commit av artikkelseksjon uten ny linje 1, ikke-ISO eller framtidsdato; `FRESHNESS_SKIP=1` for CSS-/lenke-/CTA-sveip. Testet 8 tilfeller + skarpt på julegave-committen. STEG 5 på rendret side: 14 av 14 stemmer (synlig, schema, linje 1), tidssone +0200. `page-patterns.md`-avviket «inline schema for nye artikler» er opphevet. **Åpent fra auditen:** logo 1200 px re-eksport, GTIN fra leverandør, SKU puslespill, BWT sitemap-innsending, `Product.description` 200 tegn, author `url`, llms-produktliste 13/15, kontaktsidens frakt-tekst i Admin (uverifisert AI-sammendrag sa «frakt inkludert»). Merk: den andre økten la inn to overskrifter merket «Gotcha #91» i gotchas.md (linje ~4002 og ~4023) — nummereringen bør ryddes.

---

## STATUS — 2026-09-03 kveld (**Julegave-kolleksjonen `julegaver-til-hund` opprettet, publisert og verifisert — temadel kun på preview.** Manuell kolleksjon (GID `315164229710`, MANUAL sort) laget via Admin API med tittel «Julegaver til hunden», sju produkter i låst rekkefølge (pelsfjerner → CalmBall → SnackSpin → hundeseng → Valpepakken → slikkematte-hund → puslespill-hund; alle hadde lager, ingen hoppet over), 196-ords bokmålsbeskrivelse og `seo.title`/`seo.description` sendt i samme kall (gotcha #86). Handle verifisert med friskt `collectionByHandle`-oppslag (gotcha #55). **Publisering krevde nytt scope:** `collectionCreate` gir ingen Online Store-publication (samme mekanikk som #65 for produkter), og `/collections/<h>/products.json` svarer 200 med `[]` for en upublisert kolleksjon — `shopauth` utvidet med `read_publications,write_publications`, kolleksjonen publisert med `publishablePublish` mot publication `162328674382`; gotcha #87. **Temadel** (`7534710`, `snippets/collection-catalog.liquid`): egen `when`-gren med H1/undertittel, full `collection.description` under griden (Description-blokken i `collection.json` er deaktivert, så beskrivelsen var ellers usynlig), og ItemList-JSON-LD pakket inn i `CollectionPage` for alle kolleksjonssider. Pushet til preview `149856485454`, sha256-match ved pull. **Verifisert i headless Chromium** (curl fikk 429 «Verifying your connection» etter ~15 kall): live 200 med riktig `<title>`, `description`/`og`/`twitter` ordrett som lagret, sju kort i riktig rekkefølge; preview i tillegg egen undertittel, 193 ord beskrivelse og gyldig CollectionPage → ItemList (7). **Ikke gjort, med vilje:** ikke i hovedmeny/filterbar (venter på julegave-artikkelen), ingen leveringsfrist før jul, ingen live-push av temadelen, variantrekkefølgen på pelsfjerner uendret («2-pk først» kan ikke løses på kolleksjonsnivå). **Observasjon:** CalmBall-kortets hovedbilde er TINE-yoghurtbegeret — fungerer på PDP, men er et svakt gavekort; vurder annet `featured_media` før kolleksjonen lenkes.)
...
- **⏳ Avpubliser `frontpage`-kolleksjonen (Admin UI, manuelt)** — «Home page»-kolleksjonen er tom (0 produkter), ikke referert av temaet, og noindex er bekreftet tilsiktet. Den ligger likevel i den auto-genererte sitemap.xml fordi den er publisert til Online Store. **Handling: Admin → Produkter → Samlinger → Home page → Salgskanaler → fjern Online Store.** **Korrigert 08.09.2026: `write_publications` ER i `shopauth`** (verifisert mot API-et etter re-auth), så dette kan nå gjøres fra CLI i stedet for Admin-UI. Tidligere notat sa at scopet manglet — kan alternativt gis ved å utvide scope-settet og kjøre `shopauth` på nytt. Lav prioritet: noindex vinner uansett, oppføringen gir bare en «Ekskludert av noindex»-rad i dekningsrapporten. Se BESLUTNINGER 2026-08-11 kveld + gotcha #30.
...
**Tekster som ikke lenger stemmer — popup-copyen er RETTET av Sondre 09.09.2026 (commit i `sections/newsletter-popup.liquid`, live-verifisert variant A; variant B er avslått siden 08.09 og pull-verifisert i fila):** `discount_terms` → «Gjelder alle produkter ved kjøp over 295 kr. Én bruk per kunde. Ingen utløpsdato.» · `a_eyebrow` → «15 % på ditt første kjøp» · `a_sub` → «…og 15 % på ditt første kjøp over 295 kr.» · `b_step2_sub` → «…15 % på kjøp over 295 kr følger med…» · `b_code_label` → «Bonus: 15 % over 295 kr». Schema-`info` sier nå at minstebeløpet må speiles her, og at «kan kombineres med fraktrabatt» aldri skal skrives fordi fri frakt er en fraktrate. Lista under viser hva som sto før:
