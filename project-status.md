...
## STATUS — 2026-09-08 (**Newsletter-popup skrudd AV på artikler/raseguider/blogg — på PREVIEW `149856485454`, ikke live.** Ny schema-checkbox `show_on_content` (default false) + Liquid-vakt etter variantvalget i `sections/newsletter-popup.liquid`: variant B rendres ikke på `template.name` page/article/blog; variant A (forside, PDP, kolleksjon) og all GA4-sporing uendret. Bakgrunn: 0,28 % på innholdsflater mot 4,6 % på PDP/forside. Preview hash-verifisert; Playwright med samtykke primet: forside + PDP åpner (variant A), labrador + hund-kaster-opp rendrer ikke elementet, live-kontrast åpner på begge; ingen JS-feil fra temaet (kun Shop-app CSP + wpm-pixel, pre-eksisterende). Gotcha #96 (popupen fjerner seg selv uten samtykke i headless). Skru på igjen: huk av feltet i temaeditoren, eller `"default": true`. **Venter på ok fra Sondre før live-push.** Ikke committet.)
