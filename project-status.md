...
>
> **Sist generert:** 2026-09-21 formiddag (**Turguiden i footer LIVE (oppgave 6b): «Turguiden – varsel om båndtvang» som `<li>` under «Hundetips» i `sections/footer.liquid`, via `turguiden-link` → `/turer#fylker`, `utm_medium=footer`, `utm_content=footer`, `data-mh-event="turguiden_cta_click"`/`-position="footer"`; guard-sjekk 5 vokter render-kallet. Sticky-sjekk etter #111: 13 unike sticky-regler utenom artikkel-sidebar, 8 live, 0 overlapper header eller dekker innhold på 390/1280 px. hund-smell-sidebaren festet aldri (`align-items: start` på grid, gotcha #113) — fikset, nå 120 px som alle andre. Hovedmenyen ER allerede på UTM-standarden (målt live) — docs var utdatert, rettet.** Detaljer i STATUS 2026-09-21 (6b).)
>
